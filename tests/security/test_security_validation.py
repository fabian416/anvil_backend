...
class TestCredentialSecurity:
    ...
    def test_no_hardcoded_api_keys_in_mcp_servers(self):
        ...
        mcp_servers_dir = (
            Path(__file__).parent.parent.parent / "src/app/infrastructure/mcp/servers"
        )
        ...
        if mcp_servers_dir.exists():
            for server_file in mcp_servers_dir.glob("*.py"):
                ...
                assert '"sk-' not in content, (
                    f"Hardcoded OpenAI key in {server_file.name}"
                )
                assert "'sk-" not in content, (
                    f"Hardcoded OpenAI key in {server_file.name}"
                )
                ...
                if "Authorization" in content:
                    ...
                    assert (
                        re.search(r"Authorization.*{.*api_key", content)
                        or "api_key if api_key else" in content
    ...
    def test_config_files_use_env_vars(self):
        ...
        config_file = (
            Path(__file__).parent.parent.parent / "src/app/setup/config/agent_squad.py"
        )
...
class TestDataProtection:
    ...
    def test_conversation_repository_uses_sqlalchemy(self):
        ...
        assert "session" in init_signature.parameters or "async_session" in str(
            init_signature
        )
...
class TestSecretsManagement:
    ...
    def test_docker_compose_uses_env_file(self):
        ...
        docker_compose_file = (
            Path(__file__).parent.parent.parent / "config/local/docker-compose-mcp.yml"
        )
    ...
    async def test_mcp_servers_cors_not_allow_all(self):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.options(
                "http://localhost:8081/health", headers={"Origin": "http://evil.com"}
            )
