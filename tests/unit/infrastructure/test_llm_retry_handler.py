...
class TestRetryHandlerFailureAndRetry:
    ...
    async def test_handle_retries_on_failure(self):
        ...
        mock_strategy2 = AsyncMock()
        mock_strategy2.generate.return_value = ("Fallback response", {"provider": "fallback"})
...
class TestRetryHandlerEdgeCases:
    ...
    async def test_handle_with_none_model_name(self):
        ...
        # Act
        result, metadata = await handler.handle(None, [{"role": "user", "content": "Test"}])
        
        # Assert
        assert result == "Response"
        mock_strategy.generate.assert_called_once_with(None, [{"role": "user", "content": "Test"}])
