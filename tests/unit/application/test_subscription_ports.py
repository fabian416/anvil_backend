...
class TestSubscriptionUserRepositoryPort:
    ...
    def test_subscription_user_repository_has_read_active_for_user_and_subscription(
        self,
    ):
        ...
        assert hasattr(
            SubscriptionUserRepository, "read_active_for_user_and_subscription"
        )
        ...
        for port in [
            ...
            for method_name in dir(port):
                if not method_name.startswith("_") and method_name not in ["__init__"]:
                    assert (
                        method_name.startswith("read_")
                        or method_name.startswith("find_")
                        or method_name.startswith("list_")
