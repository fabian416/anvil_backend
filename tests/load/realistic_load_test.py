...
class TestMetrics:
    ...
    def print_summary(self, phase: str = ""):
        ...
        if self.response_times:
            ...
            print(f"   P95:                 {sorted_times[min(p95_idx, len(sorted_times)-1)]*1000:.0f}ms")
            print(f"   P99:                 {sorted_times[min(p99_idx, len(sorted_times)-1)]*1000:.0f}ms")
        ...
        # Pass/Fail
        p95 = sorted_times[min(p95_idx, len(sorted_times)-1)]*1000 if self.response_times else 0

        print(f"\n✅ PASS/FAIL CRITERIA")
        print(f"   Error Rate < 5%:     {'✅ PASS' if error_rate < 5.0 else '❌ FAIL'} ({error_rate:.2f}%)")
        print(f"   P95 < 500ms:         {'✅ PASS' if p95 < 500 else '❌ FAIL'} ({p95:.0f}ms)")
        print(f"   Cache Hit Rate > 70%: {'✅ PASS' if cache_hit_rate > 70 else '⚠️  WARN'} ({cache_hit_rate:.1f}%)")

        print("\n" + "=" * 80 + "\n")


async def make_request(session: aiohttp.ClientSession, url: str, content: str, language: str = "en") -> tuple[bool, float, str]:
    ...
    try:
        async with session.post(
            ...
            if response.status == 200:
                ...
                has_content = 'agent_message' in data and 'content' in data.get('agent_message', {})
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while time.time() - start_time < duration:
            ...
            if rand < 0.1:
                ...
                intent = random.choice(['sentiment', 'trading_signals', 'price_prediction'])
            ...
            if metrics.total_requests % 10 == 0:
                ...
                print(f"   {metrics.total_requests:>3} requests | {rate:.1f} req/s | {metrics.successful}/{metrics.total_requests} success")
