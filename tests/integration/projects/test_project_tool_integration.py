...
class TestProjectTemplates:
    ...
    def test_conservative_investor_template(self):
        ...
        assert "hunter_sentiment_analysis" in template["enabled_tools"]
        assert (
            "hunter_trading_signals" not in template["enabled_tools"]
        )  # Disabled for safety
    ...
    def test_create_project_from_template(self):
        ...
        project = create_project_from_template(
            DEFI_SWING_TRADER_TEMPLATE, created_by=admin_id
        )
...
class TestProjectToolExecutor:
    ...
    def mock_ultra_executor(self):
        ...
        executor.execute_tool.return_value = (
            "Phase 3 implementation - Mock ULTRA response"
        )
        return executor
    ...
    def swing_trader_project(self):
        ...
        return create_project_from_template(
            DEFI_SWING_TRADER_TEMPLATE, created_by=uuid4()
        )
    ...
    def conservative_project(self):
        ...
        return create_project_from_template(
            CONSERVATIVE_INVESTOR_TEMPLATE, created_by=uuid4()
        )
    ...
    async def test_execute_enabled_tool(
        ...
        executor = ProjectToolExecutor(
            swing_trader_project, mock_hunter_executor, mock_ultra_executor
        )
    ...
    async def test_execute_disabled_tool_raises_error(
        ...
        executor = ProjectToolExecutor(
            conservative_project, mock_hunter_executor, mock_ultra_executor
        )

        # Conservative Investor doesn't have trading signals enabled
        with pytest.raises(ToolExecutionError) as exc_info:
            await executor.execute_tool(
                tool_name="hunter_trading_signals", parameters={"token_symbol": "ETH"}
            )
    ...
    @pytest.mark.asyncio
    async def test_portfolio_risk_tolerance_validation(
        ...
        portfolio_project = create_project_from_template(
            PORTFOLIO_MANAGER_TEMPLATE, created_by=uuid4()
        )

        executor = ProjectToolExecutor(
            portfolio_project, mock_hunter_executor, mock_ultra_executor
        )
    ...
    @pytest.mark.asyncio
    async def test_ultra_capital_validation(
        self, mock_hunter_executor, mock_ultra_executor
    ):
        ...
        executor = ProjectToolExecutor(
            arb_project, mock_hunter_executor, mock_ultra_executor
        )

        # This should raise error (exceeds capital limit)
        with pytest.raises(ToolExecutionError) as exc_info:
            await executor.execute_tool(
                tool_name="ultra_arbitrage_discovery", parameters={"capital": 200000}
            )
    ...
    async def test_arbitrage_project_allows_ultra_tools(
        ...
        executor = ProjectToolExecutor(
            project, mock_hunter_executor, mock_ultra_executor
        )

        # ULTRA tools should be allowed (will return placeholder in Phase 2)
        result = await executor.execute_tool(
            tool_name="ultra_flash_loans", parameters={"capital": 10000}
        )
...
class TestConversationProjectLinking:
    ...
    def test_conversation_without_project(self):
        ...
        conversation = Conversation.create(user_id=1, title="General Chat")
    ...
    def test_conversation_with_project(self):
        ...
        conversation = Conversation.create(
            user_id=1, title="Swing Trader Chat", project_id=project_id
        )
