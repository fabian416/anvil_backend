...
class TestAdminOverviewDashboards:
    async def test_security_dashboard_route_exists(self, client) -> None:
        """
        WHEN requesting the admin security dashboard
        THEN the API SHALL respond (200 for configured admin auth, otherwise 401/403).
        """
        response = await client.get("/api/v1/admin/security/dashboard")
        assert response.status_code in (200, 401, 403)

    async def test_chat_dashboard_route_exists(self, client) -> None:
        """
        WHEN requesting the admin chat dashboard
        THEN the API SHALL respond (200 for configured admin auth, otherwise 401/403).
        """
        response = await client.get("/api/v1/admin/chat/dashboard")
        assert response.status_code in (200, 401, 403)

    async def test_unauthenticated_returns_401_or_403(self, client) -> None:
        """
        WHEN requesting admin dashboards without auth
        THEN the API SHALL reject the request (401/403 depending on environment).
        """
        security = await client.get("/api/v1/admin/security/dashboard")
        chat = await client.get("/api/v1/admin/chat/dashboard")
        assert security.status_code in (401, 403)
        assert chat.status_code in (401, 403)

    async def test_admin_transactions_route_exists(self, client) -> None:
        """
        WHEN requesting the admin transactions endpoint
        THEN the API SHALL respond (200 for configured admin auth, otherwise 401/403).
        """
        response = await client.get("/api/v1/admin/transactions?limit=5&offset=0")
        assert response.status_code in (200, 401, 403)
