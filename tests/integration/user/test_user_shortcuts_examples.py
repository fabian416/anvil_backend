...
ACCESS_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdXRoX3Nlc3Npb25faWQiOiIxT01zYm1wNHpkZnRfYzBjLXhGUXdVUXNQLTVINXNCV0ZySjA3aVF2eGtrIiwiZXhwIjoxNzY3ODIxMDYwfQ.uUTpt-Q2N-JmG2laF_iekZjY3LV3XNOutajOBwX-oqI"
...
async def test_user_shortcut_examples_detect_correct_intent(
    ...
    if failures:
        error_msg = "\n".join(
            [
                f"❌ {f['shortcut']} ({f['intent']}): '{f['example']}' → detected as '{f.get('detected', 'error')}'"
                for f in failures
            ]
        )
...
async def test_user_shortcut_examples_comprehensive_output(
    ...
    intent_correct = sum(
        ...
        for e in r["examples"]
        if e.get("detected_intent") == r["intent"]
    )
    no_fallback = sum(
        1 for r in all_results
        for e in r["examples"]
        if not e.get("is_generic_fallback", True)
    )
    
    print(f"Total Shortcuts: {len(all_results)}")
    print(f"Total Examples: {total_examples}")
    print(f"Intent Detection: {intent_correct}/{total_examples} ({intent_correct*100//total_examples if total_examples > 0 else 0}%)")
    print(f"No Generic Fallback: {no_fallback}/{total_examples} ({no_fallback*100//total_examples if total_examples > 0 else 0}%)")
    print(f"{'='*80}\n")
    
    # Verify all examples passed
    intent_failures = [
        (r["shortcut"], e["example"], e.get("detected_intent", "unknown"), r["intent"])
        for r in all_results
        for e in r["examples"]
        if e.get("detected_intent") != r["intent"] and "error" not in e
    ]
    ...
    fallback_failures = [
        ...
        for r in all_results
        for e in r["examples"]
        if e.get("is_generic_fallback", False)
    ]
