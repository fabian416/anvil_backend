...
async def test_user_shortcut_examples_detect_correct_intent(
    ...
    await csv_tracker("user", "shortcuts", {
        "test_id": "user_shortcuts_detect_intent_001",
        "s_multistep": True,
        "input": f"Testing {len(shortcuts_data)} shortcuts with {sum(len(s['examples']) for s in shortcuts_data)} examples for intent detection",
        "output": f"All {sum(len(s['examples']) for s in shortcuts_data)} examples correctly detected their intended shortcut intents",
        "test_label_sequence": "shortcuts_intent_detection",
        "output_expected": "Correct intent detection for all shortcut examples with no false positives",
...
async def test_user_shortcut_examples_not_generic_fallback(
    ...
    # CSV tracking
    await csv_tracker("user", "shortcuts", {
        "test_id": "user_shortcuts_not_generic_002",
        "s_multistep": True,
        "input": f"Testing {sum(len(s['examples']) for s in shortcuts_data)} shortcut examples for meaningful, contextually relevant responses",
        "output": f"All {sum(len(s['examples']) for s in shortcuts_data)} examples returned specific, contextually relevant responses with no generic fallback",
        "test_label_sequence": "shortcuts_no_generic_fallback",
        "output_expected": "Contextually relevant responses with no generic fallback messages for all shortcuts",
        "status": "PASS" if not failures else "FAIL",
        "date": datetime.utcnow().isoformat(),
        "quality": validation.confidence if validation else None,
        "qa_status": validation.verdict if validation else "SKIPPED",
...
async def test_user_shortcut_examples_have_meaningful_content(
    ...
    # CSV tracking
    await csv_tracker("user", "shortcuts", {
        "test_id": "user_shortcuts_meaningful_content_003",
        "s_multistep": True,
        "input": f"Testing {sum(len(s['examples']) for s in shortcuts_data)} shortcut examples for meaningful, intent-specific content (50+ chars, keywords)",
        "output": f"All {sum(len(s['examples']) for s in shortcuts_data)} examples returned responses meeting length requirements with intent-specific keywords",
        "test_label_sequence": "shortcuts_content_quality",
        "output_expected": "All shortcut responses meet minimum length (50+ chars) and contain intent-specific keywords",
        "status": "PASS" if not failures else "FAIL",
        "date": datetime.utcnow().isoformat(),
        "quality": validation.confidence if validation else None,
        "qa_status": validation.verdict if validation else "SKIPPED",
