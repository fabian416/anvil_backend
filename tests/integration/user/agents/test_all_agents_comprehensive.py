...
SIMPLE_QUERIES = [
    ...
    {
        "id": "chat_003",
        "input": "write a poem about Ethereum gas fees",
        "expected_agent": "chat",
        "category": "core",
        "subcategory": "chat",
    },
    {
        "id": "chat_004",
        "input": "tell me a joke about crypto",
        "expected_agent": "chat",
        "category": "core",
        "subcategory": "chat",
    },
    ...
    # Hunter AI Agent (Market Data)
    {
        ...
        "category": "core",
        "subcategory": "hunter_ai",
    },
    {
        "id": "hunter_002",
        "input": "ETH price today",
        "expected_agent": "hunter_ai",
        "category": "core",
        "subcategory": "hunter_ai",
    },
    ...
    # Risk Analyzer Agent
    {
        ...
        "category": "core",
        "subcategory": "risk_analyzer",
    },
    {
        "id": "risk_002",
        "input": "analyze risk of Compound protocol",
        "expected_agent": "risk_analyzer",
        "category": "core",
        "subcategory": "risk_analyzer",
    },
    # DeFi Yield Agent
    {
        ...
        "category": "core",
        "subcategory": "defi_yield",
    },
    {
        "id": "yield_002",
        "input": "top APY opportunities",
        "expected_agent": "defi_yield",
        "category": "core",
        "subcategory": "defi_yield",
    },
    {
        "id": "yield_003",
        "input": "show me high yield farms",
        "expected_agent": "defi_yield",
        "category": "core",
        "subcategory": "defi_yield",
    },
    ...
    # Transaction History Agent (Authenticated)
    {
        ...
        "category": "authenticated",
        "subcategory": "transaction_history",
    },
    {
        "id": "history_002",
    ...
    # Swap Workflow (can use moonpay_swap_flow_handler OR swap_workflow agent)
    {
        ...
        "expected_handler": "moonpay_swap_flow_handler",
        "category": "workflow",
        "subcategory": "swap",
        "is_multi_step": True,
    },
...
MULTISTEP_WORKFLOWS = [
    ...
    # Transfer Workflow - Full Flow
    {
        ...
        "steps": [
            {
                "input": "send 10 USDC to 0x742d35Cc6634C0532925a3b844Bc454e4438f44e",
                "expected_agent": "transfer_workflow",
                "requires_execute": False,
            },
            {
    ...
    async def test_all_simple_queries(self, authenticated_client, all_agents_reporter):
        ...
        for test_case in SIMPLE_QUERIES:
            ...
            conv_id = await create_conversation(
                authenticated_client, title=f"Test {test_case['id']}"
            )
            ...
            result = TestResult(
                ...
                execute_action_type=parsed.get("execute_data", {}).get(
                    "action_type", ""
                )
                if parsed.get("execute_data")
                else "",
            ...
            print(
                f"{status_emoji} {test_case['id']}: {test_case['input'][:40]}... → {display_agents} ({response_time}ms)"
            )
...
class TestComplexQueries:
    ...
    async def test_all_complex_queries(self, authenticated_client, all_agents_reporter):
        ...
        for test_case in COMPLEX_QUERIES:
            ...
            conv_id = await create_conversation(
                authenticated_client, title=f"Complex {test_case['id']}"
            )
            ...
            print(
                f"{status_emoji} {test_case['id']}: {test_case['input'][:40]}... → {actual_agents} ({response_time}ms)"
            )
...
class TestMultiStepWorkflows:
    ...
    async def test_all_multistep_workflows(
        self, authenticated_client, all_agents_reporter
    ):
        """Run all multi-step workflow tests."""
        for workflow in MULTISTEP_WORKFLOWS:
            # Create new conversation for the workflow
            conv_id = await create_conversation(
                authenticated_client, title=f"Workflow {workflow['id']}"
            )
            ...
            for step_num, step in enumerate(workflow["steps"], 1):
                ...
                print(
                    f"{status_emoji} {workflow['id']} Step {step_num}/{total_steps}: {step['input'][:30]}... → {actual_agents} {exec_marker} ({response_time}ms)"
                )
...
class TestMultilingualSupport:
    ...
    async def test_all_multilingual_queries(
        self, authenticated_client, all_agents_reporter
    ):
        ...
        for test_case in MULTILINGUAL_QUERIES:
            ...
            lang_flag = {"es": "🇪🇸", "pt": "🇧🇷", "zh": "🇨🇳", "en": "🇺🇸"}.get(
                language, "🌐"
            )
            print(
                f"{status_emoji} {lang_flag} {test_case['id']}: {test_case['input'][:30]}... → {actual_agents} ({response_time}ms)"
            )
