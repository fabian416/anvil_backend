...
class TestHyperliquidSpotTokens:
    ...
    def test_meme_tokens_in_hyperliquid(self):
        ...
        for token in meme_tokens:
            assert token in HYPERLIQUID_SPOT_TOKENS, (
                f"{token} should be in HYPERLIQUID_SPOT_TOKENS"
            )
    ...
    def test_major_tokens_not_in_hyperliquid(self):
        ...
        for token in major_tokens:
            assert token not in HYPERLIQUID_SPOT_TOKENS, (
                f"{token} should NOT be in HYPERLIQUID_SPOT_TOKENS"
            )
...
class TestMajorTokensRequireDex:
    ...
    def test_major_tokens_present(self):
        ...
        for token in expected:
            assert token in MAJOR_TOKENS_REQUIRE_DEX, (
                f"{token} should be in MAJOR_TOKENS_REQUIRE_DEX"
            )
...
class TestFetchQuoteProviderRouting:
    ...
    def mock_hyperliquid(self):
        ...
        client = AsyncMock()
        client.get_spot_quote = AsyncMock(
            return_value=MockSpotQuote(
                from_token="USDC",
                to_token="PURR",
                from_amount=100.0,
                to_amount=50000.0,
    ...
    @pytest.mark.asyncio
    async def test_meme_token_uses_hyperliquid(
        self, agent_with_all_providers, mock_hyperliquid
    ):
...
class TestSwapWorkflowEndToEnd:
    ...
    @pytest.mark.asyncio
    async def test_workflow_with_preset_params_uses_hyperliquid(
        self, agent, mock_hyperliquid
    ):
