...
class TestKnowledgeInjectionErrors:
    ...
    async def test_invalid_token_symbol_handling(self, client: AsyncClient, llm_validator):
        ...
        if llm_validator.enabled:
            validation = await llm_validator.validate_single_response(
                ...
                test_func=self.test_invalid_token_symbol_handling,  # PHASE 3: Custom prompt generation
    ...
    async def test_malformed_api_response_handling(self, client: AsyncClient, llm_validator):
        ...
        if llm_validator.enabled:
            validation = await llm_validator.validate_single_response(
                ...
                test_func=self.test_malformed_api_response_handling,  # PHASE 3: Custom prompt generation
                additional_context={'test_category': 'info_query', 'topic': 'crypto/DeFi'}
            )
            if validation.verdict != "PASS":
                warnings.warn(
    ...
    async def test_partial_data_availability(self, client: AsyncClient, llm_validator):
        ...
        if llm_validator.enabled:
            validation = await llm_validator.validate_single_response(
                ...
                test_func=self.test_partial_data_availability,  # PHASE 3: Custom prompt generation
                additional_context={'test_category': 'info_query', 'topic': 'crypto/DeFi'}
            )
            if validation.verdict != "PASS":
                warnings.warn(
    ...
    async def test_stale_data_detection(self, client: AsyncClient, llm_validator):
        ...
        if llm_validator.enabled:
            validation = await llm_validator.validate_single_response(
                ...
                test_func=self.test_stale_data_detection,  # PHASE 3: Custom prompt generation
                additional_context={'test_category': 'info_query', 'topic': 'crypto/DeFi'}
            )
            if validation.verdict != "PASS":
                warnings.warn(
    ...
    async def test_rate_limit_on_knowledge_apis(self, client: AsyncClient, llm_validator):
        ...
        # Extract agent response from last query for validation
        agent_response = data["agent_message"]["content"]
        ...
        if llm_validator.enabled:
            validation = await llm_validator.validate_single_response(
                ...
                test_func=self.test_rate_limit_on_knowledge_apis,  # PHASE 3: Custom prompt generation
                additional_context={'test_category': 'info_query', 'topic': 'crypto/DeFi'}
            )
            if validation.verdict != "PASS":
                warnings.warn(
    ...
    async def test_timeout_on_knowledge_fetch(self, client: AsyncClient, llm_validator):
        ...
        if llm_validator.enabled:
            validation = await llm_validator.validate_single_response(
                ...
                test_func=self.test_timeout_on_knowledge_fetch,  # PHASE 3: Custom prompt generation
                additional_context={'test_category': 'info_query', 'topic': 'crypto/DeFi'}
            )
            if validation.verdict != "PASS":
                warnings.warn(
