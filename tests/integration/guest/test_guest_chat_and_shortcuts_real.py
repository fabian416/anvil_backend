...
class TestChatShortcutsReal:
    ...
    def test_get_shortcuts_english(self, sync_client: TestClient):
        ...
        for shortcut in data["shortcuts"]:
            ...
            for example in shortcut["examples"]:
                ...
                assert (
                    "example" not in example.lower() or "example" in example.lower()
                )  # Allow if part of real text
        ...
        assert (
            "Morpho" in lending["description"]
            or "morpho" in lending["description"].lower()
        )
    ...
    def test_get_shortcuts_spanish(self, sync_client: TestClient):
        ...
        lending = next(s for s in data["shortcuts"] if s["intent"] == "lending")
        assert (
            "Prestar" in lending["command"] or "prestar" in lending["command"].lower()
        )
        assert (
            "Depositar" in lending["description"]
            or "depositar" in lending["description"]
        )

        # Verify examples are in Spanish
        assert any(
            "USDC" in ex or "ETH" in ex for ex in lending["examples"]
        )  # Token names stay same
...
class TestGuestChatReal:
    ...
    async def test_guest_chat_general_message(self, test_app):
        ...
        assert (
            "DeFi" in agent_content
            or "AI" in agent_content
            or "assistant" in agent_content.lower()
        )
    ...
    async def test_guest_chat_sentiment_analysis_real(self, test_app):
        ...
        if "enrichment" in data and data["enrichment"]:
            ...
            assert (
                "token" in enrichment
                or "sources" in enrichment
                or "hunter_tool" in enrichment
            )
    ...
    async def test_guest_chat_price_prediction_real(self, test_app):
        ...
        if (
            "prediction" in data["routing"]["intent"].lower()
            or "price" in data["routing"]["intent"].lower()
        ):
            assert any(
                keyword in agent_content.lower()
                for keyword in [
    ...
    async def test_guest_chat_spanish_language(self, test_app):
        ...
        assert (
            any(word in agent_content.lower() for word in spanish_words)
            or len(agent_content) > 50
        )
    ...
    async def test_guest_chat_restricted_action(self, test_app):
        ...
        if data.get("registration_required"):
            ...
            assert (
                "signup" in data["registration_required"].get("signup_url", "").lower()
            )
