...
class TestSQLInjectionProtection:
    ...
    async def test_guest_sql_injection_in_message_content(
        ...
        for payload in malicious_payloads:
            response = await client.post(
                "/api/v1/guest/chat", json={"content": payload, "language": "en"}
            )
            ...
            assert (
                "sql" not in agent_content or "sequel" in agent_content
            )  # "sequel" as in story
            assert (
                "database" not in agent_content or "data base" in agent_content
            )  # natural language

    @pytest.mark.llm_validation
    async def test_user_sql_injection_in_conversation_operations(
        self, client: AsyncClient, llm_validator
    ):
        ...
        create_response = await client.post(
            "/api/v1/conversations", json={"title": malicious_title, "language": "en"}
        )
    ...
    @pytest.mark.llm_validation
    async def test_sql_injection_in_search_parameters(
        self, client: AsyncClient, llm_validator
    ):
...
class TestXSSProtection:
    ...
    @pytest.mark.llm_validation
    async def test_guest_xss_script_tag_injection(
        self, client: AsyncClient, llm_validator
    ):
    ...
    @pytest.mark.llm_validation
    async def test_guest_xss_event_handler_injection(
        self, client: AsyncClient, llm_validator
    ):
    ...
    @pytest.mark.llm_validation
    async def test_xss_in_user_message_reflection(
        self, client: AsyncClient, llm_validator
    ):
        ...
        response = await client.post(
            "/api/v1/guest/chat", json={"content": xss_payload, "language": "en"}
        )
...
class TestCommandInjectionProtection:
    ...
    @pytest.mark.llm_validation
    async def test_shell_metacharacters_in_input(
        self, client: AsyncClient, llm_validator
    ):
...
class TestPromptInjectionProtection:
    ...
    @pytest.mark.llm_validation
    async def test_system_prompt_override_attempt(
        self, client: AsyncClient, llm_validator
    ):
        ...
        for attempt in prompt_injection_attempts:
            response = await client.post(
                "/api/v1/guest/chat", json={"content": attempt, "language": "en"}
            )
            ...
            assert (
                "arr" not in agent_content or "arbitrage" in agent_content
            )  # "arr" could be in "array"
...
class TestPayloadValidation:
    ...
    @pytest.mark.llm_validation
    async def test_oversized_message_rejection(
        self, client: AsyncClient, llm_validator
    ):
        ...
        response = await client.post(
            "/api/v1/guest/chat", json={"content": oversized_message, "language": "en"}
        )
    ...
    @pytest.mark.llm_validation
    async def test_maximum_valid_message_accepted(
        self, client: AsyncClient, llm_validator
    ):
        ...
        response = await client.post(
            "/api/v1/guest/chat", json={"content": max_valid_message, "language": "en"}
        )
...
class TestUnicodeExploits:
    ...
    async def test_zalgo_text_handling(self, client: AsyncClient, llm_validator):
        ...
        response = await client.post(
            "/api/v1/guest/chat", json={"content": zalgo_text, "language": "en"}
        )
    ...
    async def test_emoji_bomb_protection(self, client: AsyncClient, llm_validator):
        ...
        response = await client.post(
            "/api/v1/guest/chat", json={"content": emoji_bomb, "language": "en"}
        )
...
class TestRateLimitBypass:
    ...
    @pytest.mark.llm_validation
    async def test_rate_limit_metadata_present(
        self, client: AsyncClient, llm_validator
    ):
        """Test rate limit metadata is present in responses."""
        response = await client.post(
            "/api/v1/guest/chat", json={"content": "test", "language": "en"}
        )
