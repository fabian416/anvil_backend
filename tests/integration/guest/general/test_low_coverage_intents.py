...
class TestMoneyMarketIntent:
    ...
    async def test_money_market_compare_protocols(
        ...
        assert any(
            keyword in content
            for keyword in ["aave", "compound", "rate", "apy", "yield"]
        )
    ...
    async def test_money_market_best_yields(
        ...
        assert any(
            keyword in content for keyword in ["yield", "apy", "rate", "usdc", "earn"]
        )
    ...
    async def test_money_market_supply_withdraw(
        ...
        assert any(
            keyword in content for keyword in ["supply", "withdraw", "deposit", "lend"]
        )
    ...
    async def test_money_market_spanish(
        ...
        assert any(
            keyword in content
            for keyword in ["mercado", "tasa", "apy", "yield", "rate"]
        )
...
class TestSendIntent:
    ...
    async def test_send_specific_amount(
        ...
        assert any(
            keyword in content
            for keyword in ["assistant", "sign up", "register", "demo"]
        )
    ...
    async def test_send_to_ens_name(
        ...
        assert any(
            keyword in content for keyword in ["moonpay", "swap", "pairs", "crypto"]
        )
    ...
    async def test_send_max_balance(
        ...
        assert any(
            keyword in content
            for keyword in ["send", "transfer", "eth", "balance", "all"]
        )
...
class TestReceiveIntent:
    ...
    async def test_receive_show_address(
        ...
        assert any(
            keyword in content
            for keyword in ["address", "receive", "wallet", "deposit"]
        )
    ...
    async def test_receive_qr_code(
        ...
        assert any(
            keyword in content
            for keyword in ["qr", "receive", "address", "code", "wallet"]
        )
    ...
    async def test_receive_specific_token(
        ...
        assert any(
            keyword in content
            for keyword in ["receive", "usdt", "address", "ethereum", "wallet"]
        )
...
class TestBuyIntent:
    ...
    async def test_buy_with_card(
        ...
        assert any(
            keyword in content
            for keyword in ["buy", "purchase", "card", "moonpay", "usdc"]
        )
    ...
    async def test_buy_portuguese(
        ...
        assert any(
            keyword in content
            for keyword in ["comprar", "buy", "bitcoin", "btc", "cartão", "card"]
        )
...
class TestIntentEdgeCases:
    ...
    async def test_receive_multi_chain(
        ...
        assert any(
            keyword in content
            for keyword in ["receive", "address", "polygon", "usdc", "wallet"]
        )
