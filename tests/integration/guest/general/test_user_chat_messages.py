...
async def test_compare_guest_vs_user_endpoints(client: AsyncClient):
    ...
    print(
        f"Agent message content length: {len(guest_data.get('agent_message', {}).get('content', ''))}"
    )
    ...
    assert guest_data.get("agent_message", {}).get("content"), (
        "Guest should return agent message"
    )
    assert guest_data.get("routing", {}).get("intent"), (
        "Guest should return routing intent"
    )
