...
async def demo_validation():
    ...
    csv_file = Path("/home/ubuntu/anvil_backend/tests/output/guest/week1_8_input_output.csv")
    ...
    for i, row in enumerate(rows, 1):
        ...
        if "xss" in test_type.lower() or "sql" in test_type.lower():
            expected = "Should safely handle malicious input without executing the injection"
    ...
    for result in results:
        print(f"  • {result['verdict']}: {result['test'][:50]} (confidence: {result['confidence']:.2f})")
    ...
    print("  python tests/integration/chat/validate_existing_csvs.py --mode sample --sample-rate 20")
    print("")
