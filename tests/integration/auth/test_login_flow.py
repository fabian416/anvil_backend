...
class TestLoginFlow:
    ...
    def test_login_with_invalid_password_returns_auth_error(self, client):
        """
        ...
        response = client.post("/api/v1/account/login", json=invalid_credentials)
    ...
    def test_login_with_nonexistent_email_returns_error(self, client):
        ...
        response = client.post("/api/v1/account/login", json=nonexistent_credentials)

        # Should return 404 for user not found
        assert response.status_code == 404

    def test_login_with_invalid_email_format_returns_error(self, client):
        """
        ...
        response = client.post("/api/v1/account/login", json=invalid_email)
    ...
    def test_login_without_password_returns_error(self, client):
        ...
        response = client.post("/api/v1/account/login", json=missing_password)

        assert response.status_code == 422  # Pydantic validation error

    def test_login_response_includes_user_info(self, client, valid_credentials):
        """
...
class TestLoginAccountStatus:
    ...
    def test_login_with_inactive_account_returns_forbidden(self, client):
        """
        ...
        response = client.post("/api/v1/account/login", json=inactive_credentials)

        # Should return 401, 403, or 404
        assert response.status_code in (401, 403, 404)

    def test_login_with_blocked_account_returns_forbidden(self, client):
        """
        ...
        response = client.post("/api/v1/account/login", json=blocked_credentials)
    ...
    def test_login_with_unverified_email(self, client):
        ...
        response = client.post("/api/v1/account/login", json=unverified_credentials)
...
class TestLoginRateLimiting:
    ...
    def test_multiple_failed_attempts_triggers_rate_limit(self, client):
        """
    ...
    def test_successful_login_after_rate_limit_expires(self, client):
        """
...
class TestLoginErrorResponses:
    ...
    def test_error_response_has_standardized_format(self, client):
        """
        WHEN login fails
        THEN error response SHALL have detail or message field
        """
        invalid_credentials = {
        ...
        if response.status_code in (401, 403, 404):
            ...
            # Standardized errors may have "error" nested object
            has_error_info = (
                "detail" in data
                or "message" in data
                or ("error" in data and ("code" in data["error"] or "message" in data["error"]))
            )
            assert has_error_info, f"Expected error info in response: {data}"

    def test_error_response_includes_i18n_key(self, client):
        """
