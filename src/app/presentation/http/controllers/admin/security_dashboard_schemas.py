...
class SecurityScanResultSchema(BaseModel):
    ...
    vulnerabilities: VulnerabilitySummarySchema = Field(
...
class ToolScanResultSchema(BaseModel):
    ...
    vulnerabilities_found: int = Field(
        ge=0, description="Number of vulnerabilities found by this tool"
    )
...
class ScanHistorySchema(BaseModel):
    ...
    scans: List[SecurityScanResultSchema] = Field(
        description="List of historical scans"
    )
...
class SecurityDashboardSummarySchema(BaseModel):
    ...
    latest_scan: Optional[SecurityScanResultSchema] = Field(
        None, description="Most recent scan results"
    )
    ...
    overall_status: str = Field(
        description="Overall security posture (healthy/warning/critical)"
    )
