...
async def get_user_projects(
    ...
    assigned_projects = [p for p in all_projects if p.id in assigned_project_ids]
...
async def select_project(
    ...
    assignments = await assignment_repo.get_assignments_by_user(user_id)
    is_assigned = any(a.project_id == project_id and a.is_active for a in assignments)
    ...
    if not is_assigned:
        ...
        if project.visibility == "public":
            ...
            raise HTTPException(
                status_code=403, detail="User is not assigned to this project"
            )
...
async def join_project(
    ...
    if project.visibility != "public":
        raise HTTPException(status_code=403, detail="Project is not publicly available")
    ...
    already_assigned = any(
        a.project_id == project_id and a.is_active for a in assignments
    )

    if already_assigned:
        raise HTTPException(
            status_code=400, detail="User is already assigned to this project"
        )
    ...
    if project.max_users:
        ...
        if active_count >= project.max_users:
            raise HTTPException(
                status_code=400, detail="Project has reached maximum user capacity"
            )
