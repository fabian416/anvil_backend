...
class AgentRouter:
    ...
    def classify_intent(self, query: str) -> Tuple[AgentType, float]:
        ...
        confidence = min(
            best_agent[1] / 3.0, 1.0
        )  # Cap at 1.0, ~3 keywords = high confidence
    ...
    async def route(
        ...
        if self.debug_mode:
            ...
            print(
                f"[Router] Classified as: {agent_type.value} (confidence: {confidence:.2f})"
            )
        ...
        if agent_type not in self.agents:
            if self.settings.fallback_to_general:
                ...
                if AgentType.ANALYTICS in self.agents:
                    if self.debug_mode:
                        print(
                            f"[Router] {agent_type.value} agent disabled, falling back to Analytics"
                        )
