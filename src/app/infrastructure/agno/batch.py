...
class ToolCallBatcher:
    ...
    async def execute_batch_with_retry(
        ...
        for retry in range(max_retries):
            failed_calls = [
                r.tool_call for r in results if not r.success
            ]
            
            if not failed_calls:
                break
            
            logger.info(f"Retrying {len(failed_calls)} failed calls (attempt {retry + 1})")
...
if __name__ == "__main__":
    async def test_batching():
        ...
        calls = [
            ToolCall("portfolio__get_user_balance", {"user_id": "user_1", "chain_id": 1}),
            ToolCall("portfolio__get_user_balance", {"user_id": "user_2", "chain_id": 1}),
            ToolCall("portfolio__get_user_balance", {"user_id": "user_3", "chain_id": 1}),
