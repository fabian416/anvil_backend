...
from app.infrastructure.mcp.base import (
    MCPServer,
    MCPToolDescription,
    MCPToolRequest,
    MCPToolResponse,
...
class MCPServerManager:
    ...
    def register_server(
        ...
        if not server_name:
            raise ValueError(
                "Server must have either 'name' or 'server_name' attribute"
            )
        ...
        print(
            f"[Manager] Registered server: {server_name} (port {port}, {len(server.tools)} tools)"
        )
    ...
    def _register_routes(self):
        ...
        async def manager_info():
            ...
            return {
                ...
                "total_tools": sum(
                    len(info.server.tools) for info in self.servers.values()
                ),
            }
...
""")
    for status in manager.get_server_status():
        print(
            f"  • {status['name']:15} - Port {status['port']} - {status['tools_count']} tools"
        )
