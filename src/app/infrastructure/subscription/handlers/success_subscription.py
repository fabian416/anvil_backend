...
class SubscriptionSuccessHandler:
    ...
    async def execute(self, request: SubscriptionSuccessRequest) -> dict:
        ...
        subs_user = await self._subs_user.read_by_checkout_session_id(
        ...
        await self._subs_user.update_stripe_subscription_id(
            id_=int(subs_user["id"]), stripe_subscription_id=session["subscription"]
        )  # type: ignore[index]

        pending_payment = await self._payments.find_pending_for_subscription_user(
            subscription_user_id=int(subs_user["id"])
        )
        if pending_payment:
            await self._payments.update_status(
                id_=int(pending_payment["id"]), status="completed"
            )
