...
def reindex_knowledge_base(knowledge_base_id: str):
    ...
    async def runner(container):
        ...
        print(
            f"[Reindex] Starting reindex for KB {knowledge_base_id} ({len(documents)} docs)"
        )
...
def evaluate_auto_assignment_rules(user_id: str, user_context: dict):
    ...
    async def runner(container):
        ...
        existing_project_ids = [
            a.project_id for a in existing_assignments if a.is_active
        ]
        ...
        print(
            f"[Auto-Assignment] User {user_id}: {len(new_assignments)} new assignments"
        )
        ...
        if new_assignments:
            for assignment in new_assignments:
                ...
                matching_rules = [
                    r for r in all_rules if r.project_id == assignment.project_id
                ]
                if matching_rules and matching_rules[0].auto_switch:
                    # Auto-switch to this project
                    await assignment_repo.set_active_project(
                        user_uuid, assignment.project_id
                    )
                    print(
                        f"[Auto-Assignment] Auto-switched user {user_id} to project {assignment.project_id}"
                    )
                    break
...
def aggregate_project_analytics():
    ...
    async def runner(container):
        ...
        try:
            ...
            if isinstance(e.orig, UndefinedTable) and "project_analytics_daily" in str(
                e
            ):
                print(
                    f"[Analytics] Skipping aggregation - table 'project_analytics_daily' does not exist yet. Run migrations to create it."
                )
            else:
                raise
        except Exception as e:
...
def check_knowledge_base_health():
    ...
    async def runner(container):
        ...
        try:
            ...
            if errors:
                print(
                    f"[KB Health] Found {len(errors)} documents with processing errors:"
                )
                for row in errors:
                    print(
                        f"  - KB: {row.name}, Doc: {row.title}, Error: {row.processing_error}"
                    )
            ...
            if low_chunks:
                print(
                    f"[KB Health] Found {len(low_chunks)} documents with low chunk counts:"
                )
                for row in low_chunks:
                    print(
                        f"  - KB: {row.name}, Doc: {row.title}, Chunks: {row.chunk_count}"
                    )
            ...
            stale_result = await session.execute(
                stale_query, {"threshold": stale_threshold}
            )
            ...
            if stale:
                ...
                for row in stale:
                    print(
                        f"  - KB: {row.name}, Doc: {row.title}, Updated: {row.updated_at}"
                    )
            ...
            ) and "project_knowledge_documents" in str(e):
                print(
                    f"[KB Health] Skipping health check - table 'project_knowledge_documents' does not exist yet. Run migrations to create it."
                )
            else:
                raise
        except Exception as e:
