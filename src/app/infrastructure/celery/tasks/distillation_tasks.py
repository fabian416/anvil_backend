    ...
    async def runner(container):
        ...
        try:
            ...
            await session.execute(
                query, {"hour_start": hour_start, "hour_end": hour_end}
            )
            ...
            if isinstance(e.orig, UndefinedTable) and (
                "distillation_telemetry_hourly" in str(e)
                or "distillation_requests" in str(e)
            ):
                print(
                    f"[Telemetry] Skipping aggregation - required tables do not exist yet. Run migrations to create them."
                )
            else:
                raise
...
def cleanup_expired_cache():
    ...
    async def runner(container):
        ...
        try:
            ...
            if isinstance(e.orig, UndefinedTable) and (
                "distillation_cache_exact" in str(e)
                or "distillation_cache_semantic" in str(e)
            ):
                print(
                    f"[Cache Cleanup] Skipping cleanup - cache tables do not exist yet. Run migrations to create them."
                )
            else:
                raise
        except Exception as e:
