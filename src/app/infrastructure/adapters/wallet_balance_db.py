...
class WalletBalanceDbAdapter(WalletBalancePort):
        ...
        for wallet_row in wallet_rows:
            ...
            if token_balances_table is not None:
                token_stmt = (
                    select(
                        ...
                        func.max(token_balances_table.c.last_balance_update).label(
                            "last_update"
                        ),
                    )
                ...
                if token_rows:
                    for token_row in token_rows:
                        ...
                        balance = (
                            Decimal(str(token_row[1])) if token_row[1] else Decimal("0")
                        )
                    ...
                    logger.debug(
                        f"Got balance from token_balances: wallet_id={wallet_id}, total=${wallet_total:.2f}"
                    )
            ...
            if not chain_balances and chain_addresses_table is not None:
                ...
                for chain_row in chain_rows_legacy:
                    ...
                    balance = (
                        Decimal(str(chain_row[1])) if chain_row[1] else Decimal("0")
                    )
                ...
                if chain_balances:
                    logger.debug(
                        f"Got balance from chain_addresses (fallback): wallet_id={wallet_id}, total=${wallet_total:.2f}"
                    )
    ...
    async def get_user_balance_by_chat_user(
        ...
        stmt = select(
            chat_users_table.c.identifier, chat_users_table.c.user_type
        ...
        if user_type != "authenticated":
            logger.debug(
                f"chat_user_id={chat_user_id} is not authenticated (type={user_type})"
            )
            return None
        ...
        try:
            ...
            logger.warning(
                f"Invalid identifier '{identifier}' for chat_user_id={chat_user_id}"
            )
            return None
