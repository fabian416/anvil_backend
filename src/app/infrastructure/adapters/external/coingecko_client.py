...
class CoinGeckoClient:
    ...
    async def get_market_chart(
        ...
        return MarketChart(
            ...
            market_caps=[
                (int(ts), float(mc)) for ts, mc in data.get("market_caps", [])
            ],
            total_volumes=[
                (int(ts), float(vol)) for ts, vol in data.get("total_volumes", [])
            ],
        )
    ...
    async def get_global_data(self) -> dict[str, Any]:
        ...
        return {
            "total_market_cap": float(
                market_data.get("total_market_cap", {}).get("usd", 0)
            ),
            "total_volume": float(market_data.get("total_volume", {}).get("usd", 0)),
            "btc_dominance": float(
            ...
            "eth_dominance": float(
                market_data.get("market_cap_percentage", {}).get("eth", 0)
            ),
