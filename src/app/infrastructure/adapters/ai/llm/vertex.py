...
class VertexStrategy(LLMStrategy):
    ...
    async def generate(
        ...
        **kwargs
        ...
        if model_name.startswith("meta-llama") or "/" in model_name and "llama" in model_name.lower():
            raise ValueError(f"Model {model_name} is not a Vertex AI model. Use Gemini models (gemini-*) for Vertex AI.")
        ...
        if self.api_key:
            ...
            if not token:
                raise Exception("No authentication method available (neither API key nor OAuth token)")
            headers = {
            ...
            async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                async with session.post(endpoint, headers=headers, json=payload) as response:
                    if response.status != 200:
                        ...
                        logger.error(f"Vertex AI API Error {response.status}: {text[:500]}")
