...
def _map_model_for_fallback(
    model_name: str, is_fallback_to_deepinfra: bool = False
) -> str:
    """
    ...
    if is_fallback_to_deepinfra and model_name in VERTEX_TO_DEEPINFRA_MODEL_MAP:
        ...
        logger.info(
            f"🔄 Mapping Vertex AI model '{model_name}' → DeepInfra model '{mapped}' for fallback"
        )
        return mapped
...
class RetryHandler:
    ...
    def __init__(
        self, strategy: LLMStrategy, next_handler: Optional["RetryHandler"] = None
    ):
    ...
    async def handle(
        self, model_name: str, messages: list, **kwargs
        ...
        try:
            ...
            if is_rate_limit:
                logger.warning(
                    f"⚠️ Rate limit (429) detected, falling back to DeepInfra..."
                )
            else:
            ...
            if self._next_handler:
                ...
                fallback_model = _map_model_for_fallback(
                    model_name, is_fallback_to_deepinfra=self._is_deepinfra_fallback
                )
