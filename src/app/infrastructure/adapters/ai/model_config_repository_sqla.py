...
class ModelConfigRepositorySqla(ModelConfigRepository):
    ...
    async def get_by_provider_and_name(
        self, provider: LLMProvider, name: str
    ) -> ModelConfig | None:
        ModelsTable = mapping_registry.metadata.tables["models"]  # type: ignore
        stmt = select(ModelsTable).where(
            ModelsTable.c.provider == provider.value, ModelsTable.c.model_name == name
        )
    ...
    async def get_default_model(self, provider: LLMProvider) -> ModelConfig | None:
        ModelsTable = mapping_registry.metadata.tables["models"]  # type: ignore
        stmt = select(ModelsTable).where(
            ModelsTable.c.provider == provider.value, ModelsTable.c.is_default == True
        )
        return await self._session.scalar(stmt)
