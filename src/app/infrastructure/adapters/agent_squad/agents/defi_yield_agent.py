...
class DefiYieldAgent:
    ...
    async def execute(
        ...
        if defi_llama_client:
            ...
            try:
                logger.info(
                    "🔍 Fetching real APY data from DeFiLlama for DefiYieldAgent"
                )
                ...
                if yields:
                    ...
                    for yield_data in top_yields:
                        ...
                        protocol = (
                            str(yield_data.project or "Unknown")
                            .replace("|", "\\|")
                            .strip()
                        )
                        pool = (
                            str(yield_data.symbol or "N/A").replace("|", "\\|").strip()
                        )
                        chain = (
                            str(yield_data.chain or "Unknown")
                            .replace("|", "\\|")
                            .strip()
                        )
            ...
            logger.warning(
                "⚠️ DefiLlama client is NOT available for DefiYieldAgent (client is None)"
            )
        ...
        if (
            validated_client
            and hasattr(validated_client, "get_protocol_yields")
            and yield_data_context
        ):
            sources.append(
                create_api_source(
