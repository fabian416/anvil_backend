...
class GasOptimizerAgent:
    ...
    async def execute(
        ...
        if web3_client and not hasattr(web3_client, "get_gas_price"):
            ...
            logger.warning(
                f"⚠️ Wrong object injected for web3_client: {type(web3_client)}. Creating client directly."
            )
            ...
            infura_key = os.getenv("INFURA_API_KEY") or os.getenv(
                "RPC_INFURA_API_KEY", ""
            )
            if alchemy_key or infura_key:
                from app.infrastructure.adapters.external.web3_client import (
                    Web3Client,
                    Chain,
                )
        ...
        if web3_client:
            try:
                ...
                logger = logging.getLogger(__name__)
                logger.info(
                    "🔍 Fetching real-time gas prices from Web3Client for GasOptimizerAgent"
                )
                ...
                if any(
                    ...
                    for word in ["ethereum", "eth", "mainnet", "ethereum mainnet"]
                ):
                    chains_to_check.append("ethereum")
                if any(
                    word in message_lower
                    for word in ["polygon", "matic", "polygon network"]
                ):
                    ...
                    try:
                        ...
                        if chain_name == "ethereum":
                            ...
                            if gas_price:
                                ...
                                gas_price_context += (
                                    f"- Base Fee: {gas_price.base_fee_gwei:.1f} gwei\n"
                                )
                                ...
                                logger.info(
                                    f"✅ Fetched {chain_name} gas prices: Slow={slow_gwei:.1f}, Standard={standard_gwei:.1f}, Fast={fast_gwei:.1f} gwei"
                                )
                        else:
                        ...
                        logger.warning(
                            f"⚠️ Failed to fetch {chain_name} gas prices: {chain_error}"
                        )
                        continue
                ...
                logger = logging.getLogger(__name__)
                logger.warning(
                    f"⚠️ Failed to fetch gas prices: {e}, continuing with LLM-only response"
                )
                gas_price_context = ""
        ...
        validated_web3_client = (
            web3_client if "web3_client" in locals() else self._web3_client
        )
        if (
            validated_web3_client
            and hasattr(validated_web3_client, "get_gas_price")
            and gas_price_context
        ):
            ...
            if (
                "bsc" in gas_price_context.lower()
                or "binance" in gas_price_context.lower()
            ):
                chains_mentioned.append("BSC")

            chain_name = (
                ", ".join(chains_mentioned) if chains_mentioned else "Multiple Chains"
            )
            sources.append(
                create_blockchain_source(
                    chain=chain_name,
