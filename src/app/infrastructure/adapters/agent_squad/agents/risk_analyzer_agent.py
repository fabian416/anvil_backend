...
class RiskAnalyzerAgent:
    ...
    async def execute(
        ...
        if defi_llama_client and not hasattr(defi_llama_client, "get_protocol_tvl"):
            ...
            logger.warning(
                f"⚠️ Wrong object injected for defi_llama_client: {type(defi_llama_client)}. Creating client directly."
            )
        ...
        if defi_llama_client:
            ...
            try:
                logger.info(
                    "🔍 Fetching protocol risk data from DeFiLlama for RiskAnalyzerAgent"
                )
                ...
                if protocol_filter:
                    try:
                        protocol_tvl = await defi_llama_client.get_protocol_tvl(
                            protocol_filter
                        )
                        ...
                        if protocol_tvl:
                            ...
                            risk_data_context += (
                                f"**{protocol_filter.upper()} Protocol:**\n"
                            )
                            risk_data_context += (
                                f"- Total TVL: ${protocol_tvl.tvl:,.0f}\n"
                            )
                            ...
                            logger.info(
                                f"✅ Fetched {protocol_filter} TVL data: ${protocol_tvl.tvl:,.0f}"
                            )
                ...
                try:
                    ...
                    if all_protocols:
                        ...
                        top_protocols = sorted(
                            all_protocols, key=lambda p: p.tvl, reverse=True
                        )[:10]
                        ...
                        for i, protocol in enumerate(top_protocols, 1):
                            risk_data_context += (
                                f"{i}. {protocol.name}: ${protocol.tvl:,.0f} TVL"
                            )
                            if protocol.change_7d:
                                change_sign = "+" if protocol.change_7d >= 0 else ""
                                risk_data_context += (
                                    f" ({change_sign}{protocol.change_7d:.1f}% 7d)"
                                )
                            risk_data_context += "\n"

                        logger.info(
                            f"✅ Fetched {len(all_protocols)} protocols from DeFiLlama"
                        )
                except Exception as protocols_error:
                    logger.warning(
                        f"⚠️ Failed to fetch all protocols: {protocols_error}"
                    )

            except Exception as e:
                logger.error(
                    f"❌ Failed to fetch DeFiLlama risk data: {e}", exc_info=True
                )
                risk_data_context = ""
        else:
            logger.warning(
                "⚠️ DefiLlama client is NOT available for RiskAnalyzerAgent (client is None)"
            )
        ...
        validated_client = (
            defi_llama_client
            if "defi_llama_client" in locals() and defi_llama_client
            else self._defi_llama_client
        )
        if (
            validated_client
            and hasattr(validated_client, "get_protocol_tvl")
            and risk_data_context
        ):
            sources.append(
                create_api_source(
