...
class LendingBorrowingAgentAave:
    ...
    async def execute(
        ...
        return AgentResponse(
            ...
            metadata={
                ...
                "liquidation_risk": position["health_factor"] < 1.5
                if position
                else False,
            },
        )
    ...
    async def _get_user_position(self, wallet_address: str | None) -> dict | None:
        ...
        if self._aave_client:
            try:
                position_data = await self._aave_client.get_user_position(
                    wallet_address
                )
                ...
                if position_data:
                    ...
                    return {
                        ...
                        "available_borrow_usd": float(
                            position_data.get("available_borrows_usd", 0)
                        ),
                        "ltv": float(position_data.get("ltv", 0)),
                        "liquidation_threshold": float(
                            position_data.get("liquidation_threshold", 0)
                        ),
    ...
    async def _generate_lending_report(
        ...
        health_factor_display = (
            "∞" if health_factor == float("inf") else f"{health_factor:.2f}"
        )
...
"""
        ...
        for asset in position["collateral_assets"]:
            report += (
                f"  • {asset['amount']} {asset['token']} (${asset['value_usd']:,.2f})\n"
            )
