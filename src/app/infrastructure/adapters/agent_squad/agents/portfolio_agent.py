...
class PortfolioAgent:
    ...
    async def execute(
        ...
        if user_context:
            ...
            logger.info(
                f"📊 PortfolioAgent using real user data for authenticated user"
            )
        ...
        if self._coingecko_client:
            try:
                ...
                logger.info(
                    "🔍 Fetching real-time prices from CoinGecko for PortfolioAgent"
                )
                ...
                if not tokens_to_fetch and any(
                    word in message_lower
                    for word in ["price", "prices", "portfolio", "value", "valuation"]
                ):
                    tokens_to_fetch = ["bitcoin", "ethereum", "usd-coin"]

                if tokens_to_fetch:
                    prices = await self._coingecko_client.get_prices_bulk(
                        tokens_to_fetch
                    )
                    ...
                    if prices:
                        ...
                        for token_id, price_data in prices.items():
                            if price_data:
                                price_data_context += (
                                    f"- {token_id.upper()}: ${price_data.usd:,.2f}"
                                )
                                if price_data.change_24h is not None:
                                    change_sign = (
                                        "+" if price_data.change_24h >= 0 else ""
                                    )
                                    price_data_context += f" ({change_sign}{price_data.change_24h:.2f}% 24h)"
                                price_data_context += "\n"

                        logger.info(
                            f"✅ Fetched prices for {len(prices)} tokens from CoinGecko"
                        )
                    else:
                ...
                logger.warning(
                    f"⚠️ Failed to fetch CoinGecko prices: {e}, continuing with LLM-only response"
                )
                price_data_context = ""
...
Analyze and respond using the portfolio data provided above."""
        ...
        system_prompt = (
            self._get_authenticated_system_prompt()
            if is_authenticated
    ...
    def _extract_user_context(
        ...
        if conversation_context.user_metadata.get("is_authenticated"):
            return {
                ...
                "wallet_address": conversation_context.user_metadata.get(
                    "wallet_address"
                ),
                ...
                if conversation_context.user_metadata.get("wallet_address")
                else None,
            }
    ...
    def _build_portfolio_context(self, user_context: dict[str, Any]) -> str:
        ...
        if total_value == 0:
            ...
            lines.append("")
            lines.append(
                "• 💳 **Buy crypto** - Purchase USDC instantly with card, Apple Pay, or Google Pay"
            )
            lines.append(
                '• 📥 **Receive crypto** - Transfer from another wallet (say "my wallet address")'
            )
            lines.append("")
        ...
        if total_value < 50:
            ...
            if has_stablecoins:
                lines.append(
                    f"• 💰 **Earn yield** - Deposit your ${stablecoin_value:.2f} USDC to earn ~8-12% APY"
                )
        ...
        if total_value < 1000:
            ...
            if has_stablecoins:
                lines.append(
                    f"• 💰 **Earn yield on stables** - Deposit ${stablecoin_value:.2f} into Morpho or Aave for 8-12% APY"
                )
                lines.append('  → Say "deposit USDC to Morpho" or "compare USDC rates"')

            lines.append(
                "• 🔄 **Swap & diversify** - Trade between ETH, stablecoins, and trending tokens"
            )
            lines.append('  → Say "swap" to get started')

            lines.append(
                "• 📊 **Compare money markets** - Find the best lending rates across protocols"
            )
            lines.append('  → Say "compare rates" for current APYs')

            lines.append(
                "• 📈 **Research opportunities** - Ask about any token, protocol, or strategy"
            )
            ...
            if total_value < 500:
                lines.append("")
                lines.append(
                    '• 💳 **Grow portfolio** - Say "buy crypto" to add more funds'
                )
        ...
        if top_holdings:
            ...
            for holding in top_holdings[:5]:
                ...
                lines.append(
                    f"• {symbol}: {amount:,.4f} (${value_usd:,.2f} - {pct:.1f}%)"
                )
        ...
        if has_stablecoins:
            lines.append(
                f"• 💰 **Optimize yield** - You have ${stablecoin_value:,.2f} in stables earning nothing"
            )
            lines.append(
                '  → Say "compare USDC rates" to find best APY (currently 8-15%)'
            )
            lines.append('  → Say "deposit USDC to Morpho" for optimized lending')

        lines.append(
            "• 📊 **Money market arbitrage** - Compare rates across Morpho, Aave, Compound"
        )
        ...
        lines.append(
            "• 🔍 **Risk analysis** - Evaluate protocol risks and concentrations"
        )
        lines.append('  → Ask "analyze my portfolio risk"')

        lines.append(
            "• 💸 **Transfer funds** - Move assets to other wallets or protocols"
        )
