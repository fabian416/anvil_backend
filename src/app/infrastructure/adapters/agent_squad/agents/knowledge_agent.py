...
class KnowledgeAgent:
    ...
    def _build_messages(
        ...
        # For simple "what is X?" questions, don't include conversation history to avoid topic mixing
        # Only include history if it's a follow-up question (very recent, same conversation)
        is_simple_question = (
            message.value.lower().startswith(("what is", "que es", "qué es", "what are", "que son", "qué son")) or
        ...
        # Add current message with explicit instruction to focus only on this question
        instruction_suffix = ""
...
2. **DeFi Concepts**
   ...
   - Yield farming
   - Staking
   - Lending protocols (supply assets to earn yield)
   - Borrowing protocols (general DeFi concept, but Anvil does NOT support borrowing)
   - Stablecoins
   - Governance tokens
   - DAOs
