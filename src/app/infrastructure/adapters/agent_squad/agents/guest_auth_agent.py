...
class GuestAuthAgent:
    ...
    async def execute(
        ...
        language = (
            conversation_context.user_metadata.get("language", "en")
        ...
        try:
            ...
            logger.warning(
                f"Error generating contextual message: {e}, falling back to simple message"
            )
    ...
    def _detect_knowledge_intent(self, query: str) -> str:
        ...
        if any(
            kw in query_lower
            for kw in ["buy crypto", "purchase", "buy with card", "on-ramp"]
        ):
            return "BUY"

        # Send queries
        if any(
            kw in query_lower for kw in ["send crypto", "transfer tokens", "send to"]
        ):
            return "SEND"
        ...
        if any(
            kw in query_lower
            for kw in ["portfolio", "balance", "holdings", "my assets"]
        ):
            return "PORTFOLIO"
    ...
    def _format_knowledge_context(self, knowledge_dict: dict) -> str:
        ...
        if "competitive_advantages" in knowledge_dict:
            ...
            if isinstance(advantages, list):
                context_parts.append(
                    f"Advantages: {', '.join(advantages[:3])}"
                )  # First 3
