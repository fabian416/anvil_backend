...
class ExecutionAgentPrivy:
    ...
    async def execute(
        ...
        logger.info(f"🔍 Parsed intent: action={intent_response.get('action')}, from_token={intent_response.get('from_token')}, to_token={intent_response.get('to_token')}, amount={intent_response.get('amount')}")
        ...
        if oneinch_client and not hasattr(oneinch_client, 'get_swap_quote'):
            ...
            logger.warning(f"⚠️ Wrong object injected for oneinch_client: {type(oneinch_client)}. Creating client directly.")
        ...
        if oneinch_client and intent_response.get("action") == "swap":
            try:
                import logging
                logger = logging.getLogger(__name__)
                logger.info("🔍 Fetching real swap quote from OneInch for ExecutionAgent")
                ...
                if from_token_addr and to_token_addr:
                    ...
                    to_amount_readable = int(quote.to_amount) / 1e18  # Assuming 18 decimals
                    ...
                    logger.info(f"✅ Fetched 1inch quote: {from_amount_readable} {from_token_symbol} → {to_amount_readable} {to_token_symbol}")
                else:
                    logger.warning(f"⚠️ Token addresses not found for {from_token_symbol} or {to_token_symbol}")
                    
            except Exception as e:
                import logging
                logger = logging.getLogger(__name__)
                logger.warning(f"⚠️ Failed to fetch 1inch quote: {e}, continuing without real quote")
                swap_quote_context = ""
        ...
        validated_oneinch_client = oneinch_client if 'oneinch_client' in locals() else self._oneinch_client
        if validated_oneinch_client and hasattr(validated_oneinch_client, 'get_swap_quote') and intent_response.get("action") == "swap" and swap_quote_context:
...
"""
        ...
        if not isinstance(response, dict):
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"⚠️ Intent parsing returned non-dict: {type(response)}, converting to dict")
    ...
    async def _build_execution_response(
        ...
        is_informational_query = (
            ...
            any(kw in message_lower for kw in ["what type", "what types", "what can", "can i make", "explain", "how do", "how does", "what swaps"])
        )
...
I'll provide real-time quotes with rates, gas costs, and execution details."""
            ...
            if swap_quote_context:
                ...
                base_response += f"- Expected output: {quote['to_amount']:.4f} {to_token}\n"
