...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('draft', 'active', 'paused', 'archived')", name='valid_status'),
        sa.CheckConstraint("visibility IN ('public', 'private', 'invite_only')", name='valid_visibility'),
    )
    
    op.create_index('idx_projects_status', 'projects', ['status'], postgresql_where=sa.text("status = 'active'"))
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['knowledge_base_id'], ['project_knowledge_bases.id'], ondelete='CASCADE'),
        sa.CheckConstraint("doc_type IN ('guide', 'faq', 'reference', 'data', 'announcement')", name='valid_doc_type'),
    )
    
    op.create_index('idx_knowledge_docs_kb', 'project_knowledge_documents', ['knowledge_base_id'])
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['document_id'], ['project_knowledge_documents.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['knowledge_base_id'], ['project_knowledge_bases.id'], ondelete='CASCADE'),
    )
    
    # Convert embedding column to vector type
    op.execute('ALTER TABLE project_knowledge_chunks ALTER COLUMN embedding TYPE vector(1536) USING embedding::vector(1536)')
    ...
    op.create_index('idx_knowledge_chunks_kb', 'project_knowledge_chunks', ['knowledge_base_id'])
    ...
    op.create_table(
        ...
        sa.CheckConstraint("assignment_type IN ('auto', 'manual', 'self')", name='valid_assignment_type'),
    )
    
    op.create_index('idx_assignments_user', 'user_project_assignments', ['user_id'], postgresql_where=sa.text('is_active = true'))
    op.create_index('idx_assignments_project', 'user_project_assignments', ['project_id'], postgresql_where=sa.text('is_active = true'))
    ...
    op.create_table(
        ...
        sa.Column('activated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('NOW()')),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['project_id'], ['projects.id'], ondelete='CASCADE'),
        sa.CheckConstraint("status IN ('pending', 'accepted', 'expired', 'revoked')", name='valid_invitation_status'),
    )
    ...
    op.create_index('idx_project_sessions_project', 'project_chat_sessions', ['project_id', sa.text('created_at DESC')])
    op.create_index('idx_project_sessions_user', 'project_chat_sessions', ['user_id', sa.text('created_at DESC')])
    ...
    op.create_index('idx_project_analytics_date', 'project_analytics_daily', ['project_id', sa.text('date DESC')])
