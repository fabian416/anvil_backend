...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('modified_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('NOW()')),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('intent', 'variant', name='uq_static_response_intent_variant')
    )
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('uuid_generate_v4()')),
        sa.Column('cache_key', sa.String(64), nullable=False, unique=True),  # SHA256 hash
    ...
    op.create_index('idx_cache_exact_expiry', 'distillation_cache_exact', ['expires_at'])
    ...
    op.create_table(
        ...
        sa.Column('query_embedding', postgresql.ARRAY(sa.Float), nullable=False),  # Will be cast to vector in raw SQL
    ...
    op.execute('ALTER TABLE distillation_cache_semantic ALTER COLUMN query_embedding TYPE vector(1536) USING query_embedding::vector(1536)')
    ...
    op.create_table(
        ...
        # Routing Decision
        sa.Column('route_type', sa.String(20), nullable=False),  # REJECT, CACHE, STATIC, LIGHT_LLM, FULL_LLM
    ...
    op.create_index('idx_distill_requests_user', 'distillation_requests', ['user_id', sa.text('created_at DESC')])
    op.create_index('idx_distill_requests_route', 'distillation_requests', ['route_type', sa.text('created_at DESC')])
    op.create_index('idx_distill_requests_intent', 'distillation_requests', ['intent', sa.text('created_at DESC')])
