...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('data_source', sa.String(255), nullable=False),  # e.g., 'defillama', 'immunefi'
        sa.Column('crawl_type', sa.String(100), nullable=False),  # e.g., 'full', 'incremental'
        sa.Column('started_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column('completed_at', sa.DateTime(timezone=True), nullable=True),
        sa.Column('status', sa.String(50), nullable=False, default='in_progress'),  # in_progress, completed, failed
    ...
    print("   3. Test query: SELECT * FROM cypher('defi_knowledge_graph', $$ MATCH (n) RETURN n LIMIT 10 $$) as (n agtype);")
