...
def upgrade() -> None:
    ...
    postgresql.ENUM(
        "supply", "borrow", "both", name="money_market_comparison_type_enum", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM(
        "email", "push", "in_app", name="money_market_notification_channel_enum", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM(
        "rate_above",
        "rate_below",
        "rate_change_percent",
        "best_rate_available",
        name="money_market_alert_condition_enum", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM(
        "on_chain", "api", "graph", "estimated", name="money_market_data_source_enum", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM(
        "aave_v3", "compound_v3", "morpho", name="money_market_protocol_enum", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM("info", "warning", "critical", name="alert_severity_enum", create_type=False).create(
        op.get_bind()
    )
    postgresql.ENUM(
        "health_factor_low",
    ...
    postgresql.ENUM(
        ...
        "failed",
        "cancelled",
        name="loop_status_enum", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM(
        "safe",
        "caution",
        ...
        "critical",
        "liquidatable",
        name="health_factor_level_enum", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM(
        "conservative", "moderate", "aggressive", name="risk_tolerance_enum", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM("pending", "confirmed", "failed", name="transaction_status_enum", create_type=False).create(
        op.get_bind()
    )
    postgresql.ENUM(
        "supply", "withdraw", "borrow", "repay", "liquidate", name="lending_action_enum", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM("active", "closed", "liquidated", name="lending_status_enum", create_type=False).create(
        op.get_bind()
    )
    postgresql.ENUM("supply", "borrow", name="position_type_enum", create_type=False).create(op.get_bind())
    postgresql.ENUM("aave", "morpho", name="protocol_enum", create_type=False).create(op.get_bind())
    postgresql.ENUM(
        "helpful", "not_helpful", "incorrect", "offensive", "other", name="feedbacktype", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM(
        "daily_threshold",
        "weekly_threshold",
        "monthly_threshold",
        "user_spike",
        name="costalerttype", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM(
        "rate_limit", "quota_exceeded", "throttle", "timeout", name="ratelimiteventtype", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM("success", "failed", "timeout", name="agenttoolstatus", create_type=False).create(
        op.get_bind()
    )
    postgresql.ENUM(
        "pending", "running", "completed", "failed", "skipped", name="agenttaskstatus", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM(
        "pending",
        "running",
        ...
        "failed",
        "canceled",
        name="agentexecutionstatus", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM("success", "failed", "rate_limited", "fallback", name="llmstatus", create_type=False).create(
        op.get_bind()
    )
    postgresql.ENUM("inactive", "active", "deprecated", name="modelstatus", create_type=False).create(
        op.get_bind()
    )
    postgresql.ENUM(
        "openai",
    ...
    postgresql.ENUM("active", "paused", "completed", "failed", name="schedulestatus", create_type=False).create(
        op.get_bind()
    )
    postgresql.ENUM("daily", "weekly", "biweekly", "monthly", name="frequency", create_type=False).create(
        op.get_bind()
    )
    postgresql.ENUM("active", "withdrawn", "emergency_exit", name="earnstatus", create_type=False).create(
        op.get_bind()
    )
    postgresql.ENUM("open", "closed", "liquidated", name="positionstatus", create_type=False).create(op.get_bind())
    postgresql.ENUM("long", "short", name="side", create_type=False).create(op.get_bind())
    postgresql.ENUM(
        "arbitrum",
        "base",
        "ethereum",
        ...
        "bitcoin",
        "bitcoin_testnet",
        name="chaintype", create_type=False
    ).create(op.get_bind())
    postgresql.ENUM("privy", "external", "imported", name="walletprovider", create_type=False).create(
        op.get_bind()
    )
    sa.Enum(
        "chat",
