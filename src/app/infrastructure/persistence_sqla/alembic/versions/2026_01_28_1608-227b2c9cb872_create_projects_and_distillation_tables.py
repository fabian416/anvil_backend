...
def upgrade() -> None:
    ...
    if "distillation_telemetry_hourly" not in existing_tables:
        op.create_table(
            ...
            sa.Column("id", UUID(as_uuid=True), primary_key=True),
            sa.Column(
                "hour_bucket", sa.TIMESTAMP(timezone=True), nullable=False, unique=True
            ),
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_distillation_cache_semantic_expires_at",
        table_name="distillation_cache_semantic",
    )
    op.drop_table("distillation_cache_semantic")

    op.drop_index(
        "ix_distillation_cache_exact_expires_at", table_name="distillation_cache_exact"
    )
    op.drop_index(
        "ix_distillation_cache_exact_cache_key", table_name="distillation_cache_exact"
    )
    op.drop_table("distillation_cache_exact")

    op.drop_index(
        "ix_project_analytics_daily_project_date", table_name="project_analytics_daily"
    )
    op.drop_table("project_analytics_daily")

    op.drop_index(
        "ix_project_knowledge_documents_is_processed",
        table_name="project_knowledge_documents",
    )
    op.drop_index(
        "ix_project_knowledge_documents_knowledge_base_id",
        table_name="project_knowledge_documents",
    ...
    op.drop_index(
        "ix_project_knowledge_bases_project_id", table_name="project_knowledge_bases"
    )
