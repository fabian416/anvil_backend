...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # Foreign keys
        sa.Column('chat_user_id', postgresql.UUID(as_uuid=True), 
                  nullable=False),
        sa.Column('legacy_user_id', sa.Integer(), nullable=True),
        
        # Portfolio State
        sa.Column('portfolio_state', sa.String(20), nullable=False, 
                  server_default='empty'),
        sa.Column('total_balance_usd', sa.Numeric(18, 2), nullable=False, 
                  server_default='0.00'),
        ...
        sa.Column('first_active_at', sa.DateTime(timezone=True), 
        ...
        sa.Column('failed_executions', sa.Integer(), nullable=False, 
                  server_default='0'),
        sa.Column('execution_success_rate', sa.Numeric(5, 2), nullable=False, 
                  server_default='0.00'),
        ...
        sa.Column('multi_step_completed_count', sa.Integer(), nullable=False, 
                  server_default='0'),
        sa.Column('avg_messages_per_session', sa.Numeric(5, 2), nullable=False, 
                  server_default='0.00'),
        sa.Column('detected_language', sa.String(5), nullable=False, 
                  server_default='en'),
        sa.Column('language_history', postgresql.JSONB(astext_type=sa.Text()), 
                  nullable=False, server_default='[]'),
        sa.Column('most_used_agents', postgresql.JSONB(astext_type=sa.Text()), 
                  nullable=False, server_default='[]'),
        sa.Column('agent_usage_counts', postgresql.JSONB(astext_type=sa.Text()), 
                  nullable=False, server_default='{}'),
        ...
        # Processing Metadata
        sa.Column('context_updated_at', sa.DateTime(timezone=True), 
                  server_default=sa.text('CURRENT_TIMESTAMP')),
        sa.Column('next_update_eligible_at', sa.DateTime(timezone=True), 
                  server_default=sa.text('CURRENT_TIMESTAMP')),
        ...
        sa.ForeignKeyConstraint(['chat_user_id'], ['chat_users.id'], 
                                ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['legacy_user_id'], ['users.id'], 
                                ondelete='SET NULL'),
