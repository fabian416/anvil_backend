...
def map_ai_telemetry_tables() -> None:
    ...
    if "models" not in mapping_registry.metadata.tables:
        ...
        class ModelsTable:
            ...
            __table_args__ = (
                UniqueConstraint(
                    "provider", "model_name", name="unique_provider_model"
                ),
    ...
    if "deepinfra_api_metrics" not in mapping_registry.metadata.tables:
        ...
        class DeepInfraApiMetricsTable:
            ...
            time_window_start = mapped_column(
    ...
    if "agent_tools_usage" not in mapping_registry.metadata.tables:
        ...
        class AgentToolsUsageTable:
            ...
            status = mapped_column(
                ...
                default=AgentToolStatus.SUCCESS,
                index=True,
            )
            execution_time_ms = mapped_column(Integer, nullable=True)
            error_message = mapped_column(Text, nullable=True)
    ...
    if "llm_cost_alerts" not in mapping_registry.metadata.tables:
        ...
        class LLMCostAlertsTable:
            ...
            time_period_start = mapped_column(
                DateTime(timezone=True), nullable=False, index=True
            )
            time_period_end = mapped_column(
                DateTime(timezone=True), nullable=False, index=True
            )
