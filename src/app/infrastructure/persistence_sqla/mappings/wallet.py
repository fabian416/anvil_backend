...
def map_wallet_tables() -> None:
    ...
    class WalletsTable:
        ...
        status = mapped_column(
            Integer, default=WalletStatus.ACTIVE.value, nullable=False
        )

        # Privy configuration fields (for admin management)
        policy_ids = mapped_column(
            JSON, nullable=True, default=None
        )  # Array of policy IDs
        owner_type = mapped_column(
            String(50), nullable=True, default=None
        )  # e.g., "user", "authorization_key"
        owner_id = mapped_column(
            String(255), nullable=True, default=None
        )  # ID of the owner
        additional_signers = mapped_column(
            JSON, nullable=True, default=None
        )  # Array of signer objects
        exported_at = mapped_column(
            DateTime(timezone=True), nullable=True, default=None
        )
        imported_at = mapped_column(
            DateTime(timezone=True), nullable=True, default=None
        )
        ...
        last_balance_checked_at = mapped_column(
    ...
    class ChainAddressesTable:
        ...
        eth_balance = mapped_column(
            Numeric(30, 18), nullable=True, default=0
    ...
    class TokenBalancesTable:
        ...
        can_pay_gas = mapped_column(
            Boolean, default=False, index=True
        )  # True only for native tokens
        is_stablecoin = mapped_column(
