...
class SignUpHandler:
    ...
    async def execute(self, request_data: SignUpRequest) -> SignUpResponse:
        ...
        language = (
            Language(request_data.language) if request_data.language else Language("en")
        )
        ...
        if request_data.city_id is not None and country_id is not None:
            if not await self._city_query_gateway.exists_in_country(
                request_data.city_id, country_id
            ):
                raise CityNotFoundInCountryError(request_data.city_id, country_id)
            # Translate external city_id to internal PK for FK storage
            city_pk = await self._city_query_gateway.get_pk_in_country(
                request_data.city_id, country_id
            )
            city_id = city_pk
        ...
        await self._email_verification_repo.add(
            user_id=user.id_.value, token=token, expires_at=expires_at
        )
        try:
