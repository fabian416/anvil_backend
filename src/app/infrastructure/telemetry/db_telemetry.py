...
class ConnectionPoolStats:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "last_checkout": self.last_checkout_time.isoformat()
            if self.last_checkout_time
            else None,
            "last_checkin": self.last_checkin_time.isoformat()
...
class DatabaseMetrics:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "duration_by_type": {
                k: round(v, 2) for k, v in self.duration_by_type.items()
            },
            "queries_by_table": dict(
                sorted(self.queries_by_table.items(), key=lambda x: x[1], reverse=True)[
                    :20
                ]
            ),
            "first_query_at": self.first_query_at.isoformat()
            if self.first_query_at
            else None,
...
class DatabaseTelemetry:
    ...
    def _cleanup_old_executions(self) -> None:
        ...
        self._slow_queries = [e for e in self._slow_queries if e.start_time > cutoff][
            -100:
        ]
    ...
    def get_queries_by_table(self, table: str) -> dict[str, Any]:
        ...
        executions = [e for e in self._executions if table_lower in e.table_names]
        ...
        return {
            ...
            "error_count": sum(
                1 for e in executions if e.status != QueryStatus.SUCCESS
            ),
            "query_types": {
    ...
    def get_recent_errors(self, limit: int = 20) -> list[dict[str, Any]]:
        ...
        errors = [e for e in self._executions if e.status != QueryStatus.SUCCESS]
...
def setup_engine_telemetry(
    ...
    def handle_error(exception_context: Any) -> None:
        """Record query errors."""
        conn_id = (
            id(exception_context.connection) if exception_context.connection else 0
        )
        execution = query_contexts.pop(conn_id, None)
