...
class APITelemetry:
    ...
    def _cleanup_old_records(self):
        ...
        self._recent_calls = [c for c in self._recent_calls if c.start_time > cutoff]
    ...
    async def _check_alerts(self, ctx: APICallContext, metrics: APIMetrics):
        ...
        if (
            ...
            and metrics.error_rate > self._config.error_rate_threshold
        ):
            alerts.append({
        ...
        recent_rate_limits = [
            t
            for t in metrics.rate_limit_events
        ...
        for alert in alerts:
            ...
            if (
                last_sent is None
                or datetime.now(UTC) - last_sent > self._alert_cooldown
            ):
    ...
    def get_all_metrics(self) -> dict:
        ...
        return {
            "summary": {
                ...
                "overall_error_rate": round(total_errors / total_requests * 100, 2)
                if total_requests
                else 0,
                "total_estimated_cost_usd": round(total_cost, 4),
                "apis_tracked": len(self._metrics),
            },
            "by_api": {name: m.to_dict() for name, m in self._metrics.items()},
        }
    ...
    def get_slow_calls(
        ...
        slow_calls = [c for c in self._recent_calls if c.duration_ms >= threshold_ms]
