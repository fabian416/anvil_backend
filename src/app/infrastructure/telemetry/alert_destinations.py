...
class AlertDestinationConfig:
    ...
    def from_env(cls) -> "AlertDestinationConfig":
        ...
        return cls(
            # Email
            email_enabled=os.getenv("TELEMETRY_ALERT_EMAIL_ENABLED", "false").lower() == "true",
            email_recipients=[
                r.strip() for r in os.getenv("TELEMETRY_ALERT_EMAIL_RECIPIENTS", "").split(",")
                if r.strip()
            ],
            ...
            # Slack
            slack_enabled=os.getenv("TELEMETRY_ALERT_SLACK_ENABLED", "false").lower() == "true",
            ...
            # PagerDuty
            pagerduty_enabled=os.getenv("TELEMETRY_ALERT_PAGERDUTY_ENABLED", "false").lower() == "true",
            pagerduty_routing_key=os.getenv("TELEMETRY_ALERT_PAGERDUTY_ROUTING_KEY", ""),
            
            # Severity thresholds
            min_severity_for_email=os.getenv("TELEMETRY_ALERT_MIN_EMAIL_SEVERITY", "warning"),
            min_severity_for_slack=os.getenv("TELEMETRY_ALERT_MIN_SLACK_SEVERITY", "warning"),
            min_severity_for_pagerduty=os.getenv("TELEMETRY_ALERT_MIN_PAGERDUTY_SEVERITY", "error"),
        )
...
class ConsoleAlertDestination(AlertDestination):
    ...
    async def send(self, alert: TelemetryAlert) -> bool:
        ...
        log_method(
            f"[ALERT][{alert.source.upper()}] {alert.title}: {alert.message}"
        )
        return True
...
class AlertDispatcher:
    ...
    def _setup_destinations(self) -> None:
        ...
        if self.config.email_enabled and self.config.email_recipients:
            self._destinations.append(EmailAlertDestination(
            ...
            logger.info(f"Email alerts enabled for {len(self.config.email_recipients)} recipients")
        
        # Slack
        if self.config.slack_enabled and self.config.slack_webhook_url:
            self._destinations.append(SlackAlertDestination(
        ...
        if self.config.pagerduty_enabled and self.config.pagerduty_routing_key:
            self._destinations.append(PagerDutyAlertDestination(
