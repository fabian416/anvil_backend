...
class HybridRetrievalInteractor:
        ...
        if include_dependencies:
            try:
                ...
                context["dependencies"] = {
                    "direct": [
                        d.properties.get("name", "Unknown") for d in deps.direct
                    ],
                    "indirect": [
                        d.properties.get("name", "Unknown") for d in deps.indirect
                    ],
                    "critical": [
                        d.properties.get("name", "Unknown") for d in deps.critical
                    ],
                }
            except Exception as e:
    ...
    async def find_similar_protocols(
        ...
        similar_results = [
            r for r in similar_results if r.document.entity_id != protocol_id
        ][:limit]
