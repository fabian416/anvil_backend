...
class GenerateEmbeddingsInteractor:
    ...
    async def generate_protocol_embeddings(
        ...
        logger.info(f"Generating protocol embeddings (limit={limit}, force={force_regenerate})...")
    ...
    async def _process_protocol_embedding(
        ...
        if not force_regenerate:
            ...
            if existing:
                logger.debug(f"Skipping protocol {protocol.properties.get('name')} (already exists)")
        ...
        logger.debug(f"Generated embedding for protocol {protocol.properties.get('name')}")
    ...
    async def generate_batch_embeddings(
        ...
        logger.info(f"Generating {len(entity_ids)} {entity_type} embeddings in batch...")
