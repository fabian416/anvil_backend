...
class ContextManager:
    ...
    def get_context_summary(self, context: ConversationContext) -> str:
        ...
        if context.mentioned_tokens:
            lines.append(f"• Mentioned Tokens: {', '.join(context.mentioned_tokens[-5:])}")
        ...
        if context.is_experienced_user():
            lines.append(f"• Experience: {context.interaction_count} interactions (Experienced)")
        else:
            lines.append(f"• Experience: {context.interaction_count} interactions (New)")
    ...
    def get_agent_context(self, context: ConversationContext) -> Dict[str, Any]:
        ...
        return {
            ...
            "experience_level": "experienced" if context.is_experienced_user() else "beginner",
