...
class UserContextService:
    ...
    async def get_context_by_legacy_id(
    ...
    async def _aggregate_wallet_stats(self, chat_user_id: UUID) -> WalletStats:
        ...
        if self._wallet_balance_adapter:
            try:
                aggregate = (
                    await self._wallet_balance_adapter.get_user_balance_by_chat_user(
                        chat_user_id
                    )
                )
                ...
                if aggregate:
                    ...
                    stats.token_count = (
                        sum(w.token_count for w in aggregate.wallets)
                        if aggregate.wallets
                        else 0
                    )
                    ...
                    if (
                        ...
                        if realtime_stats.total_balance_usd > 0:
                            ...
                            stats.primary_chain = (
                                realtime_stats.primary_chain or stats.primary_chain
                            )
                            logger.info(
    ...
    async def _fetch_realtime_balance(
        ...
        try:
            ...
            chain_type = chain_map.get(
                (primary_chain or "base").lower(), ChainType.BASE
            )
    ...
    async def _aggregate_execution_stats(self, chat_user_id: UUID) -> ExecutionStats:
        ...
        try:
            ...
            if hasattr(self._chat_message_repo, "get_execution_stats"):
                exec_data = await self._chat_message_repo.get_execution_stats(
                    chat_user_id
                )
                if exec_data:
