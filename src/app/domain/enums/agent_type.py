...
class AgentType(Enum):
    ...
    TRANSACTION_HISTORY = (
        "transaction_history"  # Transaction history queries (authenticated only)
    )
    ...
    MONEY_MARKET_WORKFLOW = (
        "money_market_workflow"  # Multi-step compare & select workflow
    )
    ...
    def get_all_agents(cls) -> list["AgentType"]:
        ...
        return (
            cls.get_core_agents()
            + cls.get_authenticated_agents()
            + cls.get_enterprise_agents()
        )
