...
class UserContextAware:
    ...
    def recalculate_classifications(self) -> None:
        ...
        balance_for_classification = (
            self.wallet_total_usd
            if self.wallet_total_usd > 0
            else self.total_balance_usd
        )
        self.portfolio_state = PortfolioState.from_balance(
            balance_for_classification
        ).value
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "wallet_last_sync_at": self.wallet_last_sync_at.isoformat()
            if self.wallet_last_sync_at
            else None,
            "context_updated_at": self.context_updated_at.isoformat()
            if self.context_updated_at
            else None,
        }
