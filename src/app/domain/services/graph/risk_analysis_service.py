...
class RiskAnalysisService:
    ...
    def _calculate_risk_score(
        ...
        direct_score = sum(
            self._severity_weights.get(risk.properties.get("severity", "low"), 1.0)
    ...
    def _generate_recommendations(
        ...
        oracle_risks = [
            r
            for r in direct_risks + systemic_risks
        ...
        admin_risks = [
            r for r in direct_risks if r.properties.get("type") == "admin_key"
        ]
        if admin_risks:
            recommendations.append(
            ...
            for edge in edges:
                ...
                if criticality in ["high", "critical"] or not can_function_without:
                    cascade_scenarios.append({
                        "dependent_protocol": dependent.properties.get(
                            "name", "Unknown"
                        ),
                        "dependent_id": str(dependent.id),
                        "dependency_type": edge.properties.get(
                            "dependency_type", "unknown"
                        ),
                        "criticality": criticality,
                        "can_function_without": can_function_without,
                        "impact": "Protocol would fail if dependency fails"
                        if not can_function_without
                        else "Protocol would be severely impacted",
                    })
