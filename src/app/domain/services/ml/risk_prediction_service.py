...
class RiskPredictionService:
    ...
    async def forecast_risk(
        ...
        for day in range(1, forecast_days + 1):
            forecasted_score = max(
                0.0, min(10.0, current_risk.predicted_risk_score + (daily_change * day))
            )
    ...
    def _predict_risk_score(
        ...
        normalized = {
            ...
            "days_since_last_incident": min(
                features.days_since_last_incident / 365.0, 1.0
            ),
            "critical_dependency_count": min(
                features.critical_dependency_count / 10.0, 1.0
            ),
    ...
    def _generate_recommendations(
        ...
        if features.incident_count > 1:
            recommendations.append("⚠️ Review recent incidents and implement fixes")
        ...
        if risk_trend == RiskTrend.INCREASING:
            recommendations.append("📈 Risk is increasing - conduct thorough review")
