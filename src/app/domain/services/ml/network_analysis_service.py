...
class NetworkAnalysisService:
    ...
    async def calculate_pagerank(
        ...
        for idx, (protocol_id, score) in enumerate(
            sorted(pagerank.items(), key=lambda x: x[1], reverse=True), start=1
        ):
    ...
    async def detect_communities(
        ...
        for community_id, members in enumerate(communities):
            ...
            most_common_category = (
                max(set(categories), key=categories.count) if categories else "Mixed"
            )
    ...
    async def calculate_centrality(
        ...
        for pid in protocols:
            ...
            degree_centrality = (
                (in_degree + out_degree) / (len(protocols) - 1)
                if len(protocols) > 1
                else 0
            )
    ...
    async def _label_propagation(
        ...
        for _ in range(max_iterations):
            ...
            for protocol in protocols:
                # Get neighbor labels
                neighbors = graph["incoming"].get(protocol, []) + graph["outgoing"].get(
                    protocol, []
                )
    ...
    async def _calculate_closeness(
        ...
        degree = len(graph["incoming"].get(protocol_id, [])) + len(
            graph["outgoing"].get(protocol_id, [])
        )
