...
class User(Base):
    ...
    password_hash: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, comment="Bcrypt hash - only for ADMIN/AUDITOR"
    )
    ...
    date_of_birth: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True, comment="Must be 18+ years old"
    )
    ...
    kyc_provider: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, comment="e.g., 'persona', 'onfido'"
    )
    kyc_provider_id: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, comment="External KYC provider reference ID"
    )
    ...
    terms_version: Mapped[Optional[str]] = mapped_column(
        String(20), nullable=True, comment="e.g., 'v1.0', 'v2.1'"
    )
    ...
    funding_transactions = relationship(
        "FundingTransaction", back_populates="user", lazy="select"
    )
...
class UserProfile(Base):
    ...
    timezone: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, default="UTC", comment="e.g., 'America/New_York'"
    )
    ...
    currency_preference: Mapped[Optional[str]] = mapped_column(
        String(3), nullable=True, default="USD", comment="ISO 4217 code"
    )
