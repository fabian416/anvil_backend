...
class Setting(Base):
    ...
    value: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="Setting value (string representation)"
    )
    ...
    is_encrypted: Mapped[bool] = mapped_column(
        ...
        nullable=False,
        default=False,
        comment="Value is encrypted in database"
    )
    ...
    validation_rules: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="JSON: Validation rules (min, max, regex, etc.)"
    )
    default_value: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Default value if setting is not set"
    )
    ...
    updated_by: Mapped[Optional[str]] = mapped_column(
        String(255),
        nullable=True,
        comment="Email of admin who last updated"
    )
...
class SecurityEvent(Base):
    ...
    details_json: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="JSON: Additional event details"
    )
    ...
    resolved_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime,
        nullable=True,
        comment="When event was resolved/cleared"
    )
