...
class Subscription(Base):
    ...
    stripe_customer_id: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True, comment="Stripe customer ID (cus_...)"
    )
    stripe_price_id: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, comment="Stripe price ID (price_...)"
    )
    ...
    billing_cycle_start: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="Current billing period start"
    )
    billing_cycle_end: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, index=True, comment="Current billing period end"
    )
    ...
    cancellation_feedback: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ...
    payments = relationship(
        "SubscriptionPayment", back_populates="subscription", lazy="select"
    )
...
class SubscriptionPayment(Base):
    ...
    period_start: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="Billing period start"
    )
    period_end: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="Billing period end"
    )

    # Payment Method
    payment_method_type: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, comment="'card', 'bank_account', etc."
    )
