...
class LLMConversation(Base):
    ...
    prompt_text: Mapped[str] = mapped_column(
        Text, nullable=False, comment="User's input message"
    )
    response_text: Mapped[str] = mapped_column(
        Text, nullable=False, comment="AI's response"
    )

    # Token Usage
    input_tokens: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="Number of input tokens"
    )
    output_tokens: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="Number of output tokens"
    )
    total_tokens: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="Total tokens used"
    )

    # Cost Tracking
    cost_usd: Mapped[Decimal] = mapped_column(
        Numeric(10, 6), nullable=False, default=Decimal("0"), comment="Cost in USD"
    )

    # Performance Metrics
    latency_ms: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="Response latency in milliseconds"
    )
    ...
    recommendations_json: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="JSON: AI-generated recommendations"
    )
    ...
    agent_executions = relationship(
        "AgentExecution", back_populates="conversation", lazy="select"
    )
...
class AgentExecution(Base):
    ...
    agent_type: Mapped[str] = mapped_column(
        SQLEnum(AgentType), nullable=False, index=True, comment="Type of agent executed"
    )
    ...
    # Execution Details
    status: Mapped[str] = mapped_column(
        SQLEnum(AgentStatus), nullable=False, default=AgentStatus.PENDING, index=True
    )

    # Input/Output
    input_params: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="JSON: Input parameters for agent"
    )
    output_result: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="JSON: Agent execution results"
    )

    # Performance
    execution_time_ms: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="Total execution time in milliseconds"
    )
    llm_calls_count: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="Number of LLM API calls made"
    )
    total_tokens_used: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="Total tokens across all LLM calls"
    )
    ...
    metadata_json: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="JSON: Additional execution metadata"
    )
...
class AIModel(Base):
    ...
    # Model Information
    provider: Mapped[str] = mapped_column(
        SQLEnum(LLMProvider), nullable=False, index=True
    )
    ...
    max_tokens: Mapped[int] = mapped_column(
        Integer, nullable=False, default=8192, comment="Maximum context window"
    )
    ...
    is_default: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, comment="Is this the default model?"
    )

    # Quota Management
    quota_limit_daily: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="Daily request quota (null = unlimited)"
    )
    quota_used_today: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="Requests used today"
    )
    quota_reset_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True, comment="When daily quota resets"
    )

    # Usage Statistics
    total_requests: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="Total requests all-time"
    )
    total_tokens_used: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="Total tokens all-time"
    )
    total_cost_usd: Mapped[Decimal] = mapped_column(
        Numeric(12, 2),
        nullable=False,
    ...
    def __repr__(self) -> str:
        return (
            f"<AIModel(id={self.id}, name='{self.name}', provider='{self.provider}')>"
        )
    ...
    def quota_percent_used(self) -> Optional[Decimal]:
        ...
        if self.quota_limit_daily is None or self.quota_limit_daily == 0:
            return None
        return (
            Decimal(str(self.quota_used_today)) / Decimal(str(self.quota_limit_daily))
        ) * Decimal("100")
