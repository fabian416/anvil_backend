...
class DistillationEngine:
    ...
    async def distill(self, query: str, user_id: Optional[str] = None) -> DistillationResult:
        """
        ...
        route_type = RouteType.LIGHT_LLM if complexity in [ComplexityLevel.TRIVIAL, ComplexityLevel.SIMPLE] else RouteType.FULL_LLM
...
class ComplexityAssessor:
    ...
    def assess(self, text: str, intent: Intent) -> ComplexityLevel:
        ...
        has_comparison = any(w in text.lower() for w in ["compare", "vs", "versus", "difference"])
...
class StaticResponder:
    ...
    async def get_response(self, intent: Intent, entities: ExtractedEntities) -> Optional[str]:
...
async def main():
    ...
    for query in test_queries:
        ...
        print(f"  Complexity: {result.complexity.value if result.complexity else 'N/A'}")
