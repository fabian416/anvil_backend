...
def remove_llm_validator_blocks(content: str) -> str:
    ...
    while i < len(lines):
        ...
        if '# Optional LLM semantic validation' in line and 'if llm_validator.enabled' not in line:
        ...
        if skip_mode:
            current_indent = len(line) - len(line.lstrip()) if line.strip() else skip_indent + 4
...
def fix_indentation(content: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if line.startswith('    test_cases = [') or line.startswith('    test_phrases = ['):
