...
class BackendEndpointExtractor:
    ...
    def __init__(self, project_root: Path):
        ...
        self.controllers_dir = project_root / "src" / "app" / "presentation" / "http" / "controllers"
            ...
            for i, line in enumerate(lines):
                ...
                if match:
                    ...
                    if func_line_idx < len(lines):
                        ...
                        func_name_match = re.search(r'async\s+def\s+(\w+)\s*\(', func_line)
                        if func_name_match:
    ...
    def _extract_function_block(self, lines: List[str], start_idx: int) -> str:
        ...
        for i in range(start_idx, min(len(lines), start_idx + 50)):
            ...
            if i > start_idx and line.strip() and not line.startswith(' ' * (indent_level + 1)):
                if line.strip().startswith('def ') or line.strip().startswith('async def ') or line.strip().startswith('@'):
                    break
