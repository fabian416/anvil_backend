...
class APITemplateGenerator:
    ...
    def __init__(self, project_root: Path):
        ...
        self.coverage_report_path = project_root / "scripts" / "api_audit" / "data" / "coverage_report.json"
    ...
    def generate_template(self, endpoint: Dict) -> str:
        ...
        endpoint_name = ' '.join(part.title() for part in path_parts[3:] if part and not part.startswith('{'))
    ...
    def generate_all_templates(self):
        ...
        for priority in ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW']:
            ...
            for endpoint in endpoints:
                ...
                # /api/v1/some/path -> some-path
                filename = '-'.join(part for part in path.split('/')[3:] if part and not part.startswith('{'))
                if not filename:
                    filename = 'root'
