...
class LLMValidationAdder:
        ...
        if any(word in name_lower for word in ['price', 'cost', 'worth']):
            ...
            for token in tokens:
                if token in name_lower or token in body_lower:
                    return "price_query", {"token": token.upper() if len(token) <= 4 else token.title()}
            return "price_query", {"token": "cryptocurrency"}

        # Sentiment queries
        if any(word in name_lower for word in ['sentiment', 'think', 'opinion', 'market']):
            tokens = ['bitcoin', 'btc', 'ethereum', 'eth']
            for token in tokens:
                if token in name_lower or token in body_lower:
                    return "sentiment_query", {"token": token.upper() if len(token) <= 4 else token.title()}
            return "sentiment_query", {"token": "crypto"}

        # DeFi protocol queries
        if any(word in name_lower for word in ['protocol', 'aave', 'morpho', 'compound', 'uniswap']):
        ...
        if any(word in name_lower for word in ['what_is', 'info', 'explain']):
            ...
            for topic in topics:
                if topic in name_lower or topic in body_lower:
                    return "info_query", {"topic": topic.upper() if len(topic) <= 4 else topic.title()}
    ...
    def generate_llm_validation_code(self, test_name: str, test_body: str, indent: str = "        ") -> str:
...
'''
    ...
    def add_llm_validation_marker(self, line_num: int) -> int:
        ...
        while decorator_start > 0 and (
            self.lines[decorator_start - 1].strip().startswith('@') or
            self.lines[decorator_start - 1].strip() == ''
        ):
            decorator_start -= 1
    ...
    def process_tests(self) -> int:
        ...
        for test in reversed(tests):
            ...
            while insert_line > test['line_num'] and self.lines[insert_line].strip() == '':
                insert_line -= 1
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Add LLM validation to integration tests')
