...
async def test_entities():
    ...
    try:
        ...
        print_test(
            "ChatUser.create_guest",
            guest.user_type == UserType.GUEST and guest.identifier == "192.168.1.1",
            f"type={guest.user_type.value}, id={guest.identifier}",
        )

        auth = ChatUser.create_authenticated(
            privy_id="did:privy:123", email="test@test.com"
        )
        print_test(
            "ChatUser.create_authenticated",
            auth.user_type == UserType.AUTHENTICATED
            and auth.privy_id == "did:privy:123",
            f"type={auth.user_type.value}, privy_id={auth.privy_id}",
        )

        # Test ChatConversation
        conv = ChatConversation.create(
            user_id=guest.id, title="Test Chat", language="es"
        )
        print_test(
        ...
        print_test(
            "ChatConversation.increment_messages",
...
async def test_intent_detector():
    ...
    try:
        ...
        result = detector.detect(
            "What is the market sentiment for Bitcoin?", language="en"
        )
        print_test(
        ...
        result = detector.detect(
            "¿Cuál es el sentimiento del mercado para Ethereum?", language="es"
        )
        print_test(
        ...
        print_test(
            "Spanish balance detection (restricted)",
            result.intent == ChatIntentV2.BALANCE and result.is_restricted,
            f"intent={result.intent.value}",
        )
        ...
        print_test(
            "Portfolio detection (restricted)",
...
async def test_swap_handler():
    ...
    try:
        ...
        result = await handler.handle(
            "USDC",
            continuation_step="from_token",
            continuation_value="USDC",
            language="es",
        )
        print_test(
            "Step 2: From token",
        ...
        result = await handler.handle(
            "ETH", continuation_step="to_token", continuation_value="ETH", language="es"
        )
        print_test(
            "Step 3: To token",
        ...
        result = await handler.handle(
            "100", continuation_step="amount", continuation_value="100", language="es"
        )
        print_test(
            "Step 4: Amount (complete quote)",
...
async def test_restricted_handler():
    ...
    try:
        from app.application.chat.handlers.restricted_handler import (
            RestrictedActionHandler,
        )
...
async def test_rate_limit_config():
    ...
    try:
        ...
        print_test(
            "Guest config",
            guest_config["messages_per_hour"] == 20
            and guest_config["messages_per_day"] == 50,
        ...
        # Test feature access
        print_test(
            "Guest can access sentiment",
            can_access_feature("guest", "sentiment"),
            f"access=True",
        )

        print_test(
            "Authenticated can access all",
    ...
    try:
        ...
        messages = [
            ...
            ChatMessage.create_assistant_message(
                uuid4(), "Bitcoin sentiment is bullish.", intent="HUNTER_SENTIMENT"
            ),
        ]
        context = ConversationContext(
            messages=messages,
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "anvil_db",
                "-c",
                "SELECT table_name FROM information_schema.tables WHERE table_name LIKE 'chat_%' ORDER BY table_name;",
            ],
            capture_output=True,
            text=True,
        )
        ...
        print_test("chat_users table exists", "chat_users" in result.stdout, "Found")
        print_test(
        ...
        print_test(
            "chat_messages table exists", "chat_messages" in result.stdout, "Found"
        )
        print_test(
            "chat_rate_limits table exists",
