...
class DeploymentVerifier:
    ...
    async def check_database(self):
        ...
        try:
            ...
            missing_optional = [
                ext for ext in optional_extensions if ext not in extensions
            ]
        ...
        for name, url in mcp_servers:
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"  ❌ {name}: Server responded with status {response.status_code}"
                    )
                    all_running = False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if cache_hit_rate >= 0.60:
                    ...
                    cache_msg = (
                        f"Cache hit rate: {cache_hit_rate:.1%} (below target of 60%)"
                    )
                else:
                    cache_status = CheckStatus.WARN
                    cache_msg = (
                        f"Cache hit rate: {cache_hit_rate:.1%} (needs improvement)"
                    )
